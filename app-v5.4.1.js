...
let APP_CODE_VERSION='5.4.0';
let APP_RELEASE='5.4.0 · FORECAST 4.0';
...
function gridHealthCard(b){
 const pnl=Number(b.totalProfit??b.totalProfitPct??0), liq=Number(b.liquidationDistancePct||0);
 const cls=b.status==='CRITICAL'?'red':b.status==='WATCH'?'amber':'green';
 const gp=b.gridProfit!=null?`${b.gridProfit>=0?'+':''}${fmt(b.gridProfit,2)} USDT`:b.gridProfitAsset!=null?`${b.gridProfitAsset>=0?'+':''}${fmt(b.gridProfitAsset,2)} ${b.symbol}`:'—';
 const tp=b.trendPnl!=null?`${b.trendPnl>=0?'+':''}${fmt(b.trendPnl,2)} USDT`:b.trendPnlAsset!=null?`${b.trendPnlAsset>=0?'+':''}${fmt(b.trendPnlAsset,2)} ${b.symbol}`:'—';
 return card(`<div class="section-head"><div><div class="section-title">${b.id} · ${b.side.toUpperCase()} ${b.leverage}x</div><div class="section-note">${b.name} · ${snapshotBadge('08:40 VERIFIED')}</div></div><span class="tag ${cls}">${b.status}</span></div>
 <div class="grid2">${metric('HEALTH',b.healthScore+'/100',cls)}${metric('LIQ BUFFER',fmt(liq,2)+'%',liq<10?'red':liq<30?'amber':'green')}${metric('GRID EDGE',gp,'green')}${metric('TREND DRAG',tp,(b.trendPnl??b.trendPnlAsset??0)>=0?'green':'red')}${metric('BREAK-EVEN','$'+gridFmt(b.breakEven,b.symbol))}${metric('LIQ.','$'+gridFmt(b.liquidation,b.symbol),'red')}</div>
 <div class="grid-action-box ${cls}"><b>${b.action}</b><span>${b.reason}</span></div>`);
}
function gridView(){
 const r=DATA.pionexRisk||{}, health=(r.bots||[]).map(gridHealthCard).join('');
 const bots=['HBAR','XRP'].map(botGridCard).join('');
 const scan=['SOL','ETH','PEPE'].map(scannerCard).join('');
 return card(`<div class="eyebrow">GRID HEALTH 2.0 ${snapshotBadge('PIONEX VERIFIED')}</div><div class="forecast-main">BOT RISK CONTROL</div><div class="sub">Grid Edge · Trend Drag · Liq Buffer · Break-even · Funding · Direction Match</div>
 <div class="row"><span>Portfolio Futures Risk</span><b class="red">${r.riskLevel||'—'}</b></div><div class="row"><span>Net Direction</span><b>${r.netDirection||'—'}</b></div><p class="footer-note">${r.riskNote||''}</p>`)+health+
 card(`<div class="eyebrow">FIB GRID ENGINE 1.2 ${liveBadge('LIVE')}</div><div class="forecast-main">SWING → FIB → ACTION</div><button class="mini-grid-btn" onclick="refreshGridEngine(true)">4H SWINGS JETZT NEU LADEN</button>`)+bots+
 card(`<div class="section-head"><div><div class="section-title">COIN-M OPPORTUNITY SCANNER</div><div class="section-note">Nicht-Bitpanda Kandidaten</div></div><span class="tag cyan">LIVE</span></div>${scan||'<p class="footer-note">Scanner lädt 4H-Daten …</p>'}<p class="footer-note">Botdaten = Screenshot-Snapshot; Markt-/Swingdaten können live aktualisieren. Keine automatische Bot-Ausführung.</p>`);
}
function settings(){
