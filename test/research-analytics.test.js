...
test('ledger telemetry computes expectancy payoff and splits',()=>{
  const s={account:{startEquity:10000,equity:10100,peakEquity:10200},positions:[{status:'OPEN',riskPct:.5}],trades:[trade(200),trade(-100,{side:'SHORT',symbol:'ETHUSDT'})]};
...
test('challenger counterfactual reports avoided losers and missed winners',()=>{
...
test('comparison is research-only and exposes known audit flags',()=>{
  const base={account:{startEquity:10000,equity:9900,peakEquity:10000},trades:[trade(-100)]};
  const challenger={account:{startEquity:10000,equity:10100,peakEquity:10100},trades:[trade(100,{challengerDecision:'TRADE',challengerConfidence:80,challengerRegime:'BULL'})],counterfactuals:[]};
  const regime={account:{startEquity:10000,equity:10020,peakEquity:10020},trades:[trade(20,{sourceSide:'SHORT',side:'LONG',regimeType:'TREND_UP',regimeScore:78,regimeDecision:'TRADE'})]};
