...
function botFeedFresh(){const age=botFeedAgeMs();return state.botFeedTimestampTrusted&&age!=null&&age<=15*60*1000}
...
function status(b){if(!liveMatched(b))return['REF','muted'];const r=risk(b),t=tpDist(b);if(t!=null&&t<=5)return['TP ZONE','tp'];if(r==null)return['LIQ CHECK','muted'];if(r<5)return['LIQ URGENT','danger'];if(r<10)return['LIQ MARGIN','danger'];if(r<18)return['LIQ WATCH','watch'];return['LIQ SAFE','safe']}
...
function okxStrip(){const ps=state.okxPositions||[];if(!ps.length)return'';return `<section class="okx-strip"><div class="okx-title"><span>OKX POSITIONS · SNAPSHOT</span><b>${ps.length} LONGS</b></div>${ps.map(x=>`<div class="okx-row"><strong>${x.symbol} · ${x.leverage}x</strong><span>Mark ${money(x.price)}</span><span>Entry ${money(x.entry)}</span><span>Liq ${money(x.liq)}</span><b class="${okxRisk(x)<25?'tone-watch':'tone-safe'}">${okxRisk(x).toFixed(1)}% Puffer</b></div>`).join('')}</section>`}
function dataTruthCard(){const matched=state.bots.filter(liveMatched).length,pnl=state.bots.filter(livePnlAvailable).length,verified=Object.values(state.priceChecks||{}).filter(x=>x&&x.verified).length,pSource=state.portfolio?.pionexSource==='PRIVATE_ACCOUNT_TOTAL'?'PRIVATE ACCOUNT':'SNAPSHOT',fresh=botFeedFresh(),age=ageText(botFeedAgeMs()),mode=fresh&&matched===state.bots.length?'FRESH':matched?'MIXED':'REFERENCE',unmatched=state.unmatchedLive||[],diag=state.liveRows===matched&&matched<state.bots.length?'Backend liefert aktuell nur '+state.liveRows+' Bot-Rows · Matcher ist nicht der Engpass.':state.liveRows>matched?(state.liveRows-matched)+' private Bot-Rows nicht gematcht'+(unmatched.length?' · '+unmatched.slice(0,4).map(x=>x.symbol+' '+x.side+' '+(x.leverage||'—')+'x').join(' / '):''):'Keine zusätzlichen Bot-Rows im privaten Feed.';return `<section class="data-truth"><div class="data-truth-head"><span>DATA TRUTH · r18</span><b class="${fresh?'tone-safe':'tone-watch'}">${mode}</b></div><div class="truth-grid"><div><span>PRIVATE API ROWS</span><b>${state.liveRows}</b></div><div><span>BOT MATCH</span><b>${matched}/${state.bots.length}</b></div><div><span>SNAPSHOT PNL</span><b>${pnl}</b></div><div><span>BOT SNAPSHOT AGE</span><b>${age}</b></div><div><span>ACTIONABLE</span><b>${state.bots.filter(actionableBot).length}</b></div><div><span>2-SOURCE PRICE</span><b>${verified}</b></div><div><span>UNMATCHED API</span><b>${Math.max(0,state.liveRows-matched)}</b></div><div><span>PORTFOLIO</span><b>${pSource}</b></div></div><small>${diag}<br>Profit-Lock/NEXT ACTION benötigen Bot-Match + Snapshot-PnL + vertrauenswürdigen Bot-Timestamp ≤15 Min.</small></section>`}
...
function command(){
 const c=critical(),p=state.portfolio||{total:0,pionex:0,bitpanda:state.manual.bitpanda,ledger:state.manual.ledger,okx:state.manual.okx},e=exposureModel(),rg=portfolioRegime(),sig=signalSummary(),sa=topSignalBot(),pp=topProfitPlan(),cr=c?risk(c):null,bd=c?beDistance(c):null,matched=state.bots.filter(liveMatched),freshMatched=botFeedFresh()?matched:[],tp=freshMatched.filter(b=>{const x=tpDist(b);return x!=null&&x<=10}),watch=freshMatched.filter(b=>{const x=risk(b);return x!=null&&x<30}).length;
...
async function sync(){try{const payload=await getJson('/api/private/dashboard'),d=payload?.data||payload,live=Array.isArray(d?.pionexRisk?.bots)?d.pionexRisk.bots.map(normalizeLive):[];state.bots=live.length?mergeReference(live):FALLBACK.map(ref=>({...ref,_liveMatched:false,_livePrice:false,_livePnl:false,_liveInvest:false,_source:'REFERENCE'}));state.liveRows=live.length;const specificTs=parseTs(d?.pionexRisk?.updatedAt||d?.pionexRisk?.snapshotAt);state.botFeedUpdatedAt=specificTs||parseTs(d?.privateUpdatedAt);state.botFeedTimestampTrusted=!!specificTs;state.botFeedSource=specificTs?'PRIVATE PIONEX SNAPSHOT':'PRIVATE STATE · LEGACY TIMESTAMP';const matched=state.bots.filter(liveMatched).length;state.source=botFeedFresh()&&matched===state.bots.length&&matched?'FRESH':matched?'MIXED':'REFERENCE';state.market=String(d?.market?.regime||d?.btcRegime?.label||d?.regime?.label||'SYNC').toUpperCase();state.portfolio=portfolioModel(d);state.syncedAt=Date.now();state.error=null}catch(e){state.error=e.message;state.source='REFERENCE';state.botFeedTimestampTrusted=false}renderHeaderTruth();go(current)}
