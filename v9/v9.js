...
function pick(b,keys){for(const k of keys){const v=num(b?.[k]);if(v!=null)return v}return null}function normalizeLive(b){const nested=b?.bot||b?.position||b?.data||{};const x={...nested,...b};return{id:String(x.id||x.botId||x.bot_id||x.name||x.symbol||'BOT'),symbol:String(x.symbol||x.asset||x.baseAsset||x.base_asset||'').replace(/[-_/]?(USDT|USDC|USD)$/,'').toUpperCase(),leverage:pick(x,['leverage','leverageX','leverage_x']),lower:pick(x,['lowerRange','rangeLower','lowerPrice','lower_price','minPrice','min_price']),upper:pick(x,['upperRange','rangeUpper','upperPrice','upper_price','maxPrice','max_price']),be:pick(x,['breakEvenPrice','breakevenPrice','break_even_price','avgEntryPrice','averageEntryPrice','breakEven','break_even','entryPrice','entry_price']),liq:pick(x,['pionexLiquidationPrice','liquidationPrice','liqPrice','liquidation_price']),tp:pick(x,['takeProfit','tpPrice','tp','take_profit_price']),price:pick(x,['currentPrice','price','markPrice','lastPrice','mark_price','last_price']),buffer:pick(x,['pionexLiqBufferPct','liqBufferPct','liquidationDistancePct']),pnl:pick(x,['totalProfitUsd','totalProfitUSDT','totalProfitUsdt','totalProfit','total_profit','total_profit_usdt','pnlUsd','unrealizedPnlUsd','unrealizedPnl','unrealized_profit','profit']),invest:pick(x,['investmentUsd','investmentUSDT','investmentUsdt','investment','investedUsd','invested','investment_usdt','invested_amount','initialInvestment','initial_investment']),profitPct:pick(x,['totalProfitPct','profitPct','pnlPct','total_profit_pct','profit_rate','profitRate']),side:String(x.side||x.direction||x.positionSide||'LONG').toUpperCase()}}
...
function mergeReference(live){
  ...
  for(const m of matches){
    ...
    refs[m.ri]={...ref,...Object.fromEntries(Object.entries(x).filter(([,v])=>v!=null&&v!=='')),side:ref.side||'LONG'};
...
async function syncIntel(){
 ...
 state.assetIntel=out
}

function risk(b){const p=num(b.price)||num(b.be),liq=num(b.liq),api=num(b.buffer);if(api!=null&&api>0&&api<100)return api;if(!(p>0&&liq>0))return null;return b.side==='SHORT'?(liq-p)/p*100:(p-liq)/p*100}
function assetPairRisk(symbol){
 const rows=state.bots.filter(b=>b.symbol===symbol),longs=rows.filter(b=>(b.side||'LONG')==='LONG'),shorts=rows.filter(b=>b.side==='SHORT');
 const notional=b=>{const p=num(b.price)||num(b.be),q=num(b.investCoin),usd=num(b.invest),lev=num(b.leverage)||1;return q!=null&&p>0?q*p*lev:(usd||0)*lev};
 const longUsd=longs.reduce((s,b)=>s+notional(b),0),shortUsd=shorts.reduce((s,b)=>s+notional(b),0),hedgePct=longUsd>0?shortUsd/longUsd*100:0;
 ...
 return{symbol,rows,longUsd,shortUsd,netUsd:longUsd-shortUsd,hedgePct,minBuffer,score,label,tone,reasons,profitAction,dominantSide,bias:pi?.bias||'SYNC'};
}
function riskCockpitV2(){
 ...
 return '<section class="risk-v2"><div class="section-title"><h2>RISK COCKPIT V2</h2><small>Asset-Paare · Liq + Hedge + 15m/1h/4h + BTC-Regime</small></div><div class="risk-v2-grid">'+pairs.map(x=>'<article class="risk-v2-row"><div><strong>'+x.symbol+'</strong><span class="tag tone-'+x.tone+'">'+x.label+'</span></div><div><span>NET</span><b>'+money(x.netUsd)+'</b></div><div><span>HEDGE</span><b>'+x.hedgePct.toFixed(1)+'%</b></div><div><span>MIN LIQ</span><b>'+(x.minBuffer==null?'—':x.minBuffer.toFixed(1)+'%')+'</b></div><div><span>SIGNAL '+x.dominantSide+'</span><b class="tone-'+signalTone(x.profitAction)+'">'+x.profitAction+'</b></div><small>'+(x.reasons.join(' · ')||'kein neues Warnsignal')+'</small></article>').join('')+'</div></section>';
}
...
function botProfitPct(b){const direct=num(b.profitPct);if(direct!=null)return direct;const pnl=num(b.pnl),invest=num(b.invest);return pnl!=null&&invest>0?pnl/invest*100:null}
...
function profitLockPlan(b){
 ...
 if(pnl==null)return{code:'SYNC',label:'SYNC',tone:'muted',reducePct:0,signal,pnl,tp:t,detail:'Live-PnL fehlt · '+tpText+' · keine Aktion ableiten'};
 if(pnl<=0){
...
function botCard(b){const [st,tone]=status(b),r=risk(b),t=tpDist(b),pct=botProfitPct(b),pl=profitLockPlan(b);return `<article class="bot bot-v2"><div class="bothead"><div><h3>${b.symbol}</h3><small>${b.side||'LONG'} · ${b.leverage||'—'}x</small></div><span class="tag tone-${tone}">${st}</span></div><div class="profit-lock-v2 pl-${pl.tone}"><div class="pl-top"><span>PROFIT LOCK V2</span><b class="tone-${pl.tone}">${pl.label}</b></div><small>${pl.detail}</small></div><div class="profit-row"><div><span>INVEST</span><b>${b.invest==null?'—':money(b.invest)}</b></div><div><span>GESAMTPROFIT</span><b class="${b.pnl>0?'profit-pos':b.pnl<0?'profit-neg':''}">${b.pnl==null?'—':money(b.pnl)}${pct==null?'':' · '+(pct>=0?'+':'')+pct.toFixed(1)+'%'}</b></div></div><div class="bot-details"><span>PRICE <b>${money(b.price)}</b></span><span>BE <b>${money(b.be)}</b></span><span>LIQ <b>${money(b.liq)}</b></span><span>TP <b>${money(b.tp)}</b></span></div><div class="range-line">RANGE <b>${money(b.lower)} – ${money(b.upper)}</b></div><div class="risk"><i style="width:${Math.max(2,Math.min(100,r||0)*3)}%"></i></div><small class="distance">${r==null?'Liq —':r.toFixed(1)+'% Liq'} · ${t==null?'TP —':Math.max(0,t).toFixed(1)+'% TP'}</small></article>`}
function critical(){return [...state.bots].sort((a,b)=>(risk(a)??999)-(risk(b)??999))[0]}
function beDistance(b){const p=num(b.price)||num(b.be),be=num(b.be);return p>0&&be>0?(p-be)/be*100:null}
function assetExposureShare(symbol){const e=exposureModel(),x=state.bots.filter(b=>b.symbol===symbol).reduce((s,b)=>{const p=num(b.price)||num(b.be),coin=num(b.investCoin),usd=num(b.invest);return s+(coin!=null&&p>0?coin*p:(usd||0))},0);return e.longUsd>0?x/e.longUsd*100:0}
...
function signalSummary(){const out={hold:0,watch:0,lock:0,risk:0,sync:0};for(const b of state.bots){const a=actionForSide(state.assetIntel[b.symbol],b.side||'LONG');if(a==='RISK REVIEW')out.risk++;else if(a==='PROFIT LOCK CANDIDATE')out.lock++;else if(a==='WATCH PROFIT')out.watch++;else if(a==='HOLD')out.hold++;else out.sync++}return out}
function topSignalBot(){return state.bots.map(b=>({b,action:actionForSide(state.assetIntel[b.symbol],b.side||'LONG')})).filter(x=>x.action!=='SYNC'&&x.action!=='HOLD').sort((a,b)=>signalRank(b.action)-signalRank(a.action)||((num(b.b.profitPct)||0)-(num(a.b.profitPct)||0)))[0]||null}
function exposureModel(){const notion=b=>{const px=num(b.price)||num(b.be)||num(b.entry),coin=num(b.investCoin)||num(b.sizeCoin),margin=num(b.invest)||num(b.marginUsd),lev=num(b.leverage)||1;if(coin!=null&&px>0)return coin*px*lev;if(margin!=null)return margin*lev;return 0};const pionex=state.bots.map(b=>({...b,notional:notion(b)}));const okx=(state.okxPositions||[]).map(b=>({...b,notional:notion(b)}));const manual=(state.pionexManual||[]).map(b=>({...b,notional:num(b.notionalUsd)||notion(b)}));const rows=[...pionex,...okx,...manual];const longUsd=rows.filter(b=>(b.side||'LONG')==='LONG').reduce((x,b)=>x+b.notional,0),shortUsd=rows.filter(b=>b.side==='SHORT').reduce((x,b)=>x+b.notional,0),grossUsd=longUsd+shortUsd,netUsd=longUsd-shortUsd,coverage=longUsd>0?shortUsd/longUsd*100:0;return{longUsd,shortUsd,hedgeUsd:shortUsd,grossUsd,netUsd,coverage}}
function portfolioRegime(){const e=exposureModel(),i=state.intel;let points=0,reasons=[];if(i){if(i.price>i.ema20&&i.ema20>i.ema50){points+=2;reasons.push('BTC > EMA20/50')}else if(i.price<i.ema20){points-=2;reasons.push('BTC < EMA20')}if(i.macd4?.hist>0){points++;reasons.push('4h MACD +')}else if(i.macd4?.hist<0){points--;reasons.push('4h MACD -')}if(i.rsi4>=70){points--;reasons.push('4h RSI heiß')}else if(i.rsi4>=50){points++;reasons.push('4h RSI >50')}}const avgRisk=state.bots.length?state.bots.reduce((s,b)=>s+riskV2(b).score,0)/state.bots.length:0;if(avgRisk>=6){points-=2;reasons.push('Bot-Risiko hoch')}else if(avgRisk>=4){points--;reasons.push('Bot-Risiko erhöht')}const regime=points>=2?'RISK-ON':points<=-2?'DEFENSIVE':'NEUTRAL';const hedge=e.coverage>=15?'AUSREICHEND':e.coverage>=7?'MITTEL':'KLEIN';const tone=regime==='RISK-ON'?'safe':regime==='DEFENSIVE'?'danger':'watch';return{regime,hedge,tone,points,avgRisk,reasons}}
...
function exposureStrip(){const e=exposureModel(),tone=e.coverage>=15?'safe':e.coverage>=7?'watch':'danger';return `<section class="exposure-strip"><div><span>GROSS LONG NOTIONAL</span><b>${money(e.longUsd)}</b></div><div><span>TOTAL SHORT NOTIONAL</span><b>${money(e.hedgeUsd)}</b></div><div><span>NET EXPOSURE</span><b>${money(e.netUsd)}</b></div><div><span>SHORT / LONG</span><b class="tone-${tone}">${e.coverage.toFixed(1)}%</b></div></section>`}
...
function command(){
 const c=critical(),p=state.portfolio||{total:0,pionex:0,bitpanda:state.manual.bitpanda,ledger:state.manual.ledger,okx:state.manual.okx},e=exposureModel(),rg=portfolioRegime(),sig=signalSummary(),sa=topSignalBot(),pp=topProfitPlan(),cr=c?risk(c):null,bd=c?beDistance(c):null,tp=[...state.bots].filter(b=>{const x=tpDist(b);return x!=null&&x<=10}),watch=[...state.bots].filter(b=>{const x=risk(b);return x!=null&&x<30}).length;
 ...
 else if(sa?.action==='RISK REVIEW'){action=sa.b.symbol+' '+(sa.b.side||'LONG')+' · RISK REVIEW';actionReason=reasonsForSide(state.assetIntel[sa.b.symbol],sa.b.side||'LONG').slice(0,3).join(' · ')}
 else if(sa?.action==='PROFIT LOCK CANDIDATE'){action=sa.b.symbol+' '+(sa.b.side||'LONG')+' · PROFIT LOCK PRÜFEN';actionReason=reasonsForSide(state.assetIntel[sa.b.symbol],sa.b.side||'LONG').slice(0,3).join(' · ')}
 else if(sa?.action==='WATCH PROFIT'){action=sa.b.symbol+' '+(sa.b.side||'LONG')+' · WATCH PROFIT';actionReason=reasonsForSide(state.assetIntel[sa.b.symbol],sa.b.side||'LONG').slice(0,3).join(' · ')}
 return `<section class="portfolio-hero command-hero"><div class="hero-top"><div><span>GESAMTPORTFOLIO</span><strong>${p.total!=null?money(p.total):'PIONEX SYNC'}</strong><small>PORTFOLIO EQUITY · ungehebelt</small></div><div class="regime-pill tone-${rg.tone}">${rg.regime}</div></div><div class="source-grid"><div><span>PIONEX</span><b>${p.pionex!=null?money(p.pionex):'SYNC'}</b><small>${p.pionexSource==='PRIVATE_ACCOUNT_TOTAL'?'LIVE ACCOUNT':'VERIFIED SNAPSHOT'}</small></div><div><span>LEDGER</span><b>${money(p.ledger)}</b><small>WALLET</small></div><div><span>OKX</span><b>${money(p.okx)}</b><small>2 Futures Longs</small></div></div></section><section class="risk-cockpit"><div class="cockpit-head"><div><span>RISK COCKPIT</span><b class="tone-${c?status(c)[1]:'muted'}">${c?c.symbol+' '+(c.side||'LONG')+' '+(c.leverage||'—')+'x':'SYNC'}</b></div><div class="liq-big"><small>LIQ PUFFER</small><strong>${cr==null?'—':cr.toFixed(1)+'%'}</strong></div></div><div class="risk-kpis"><div><span>PRICE</span><b>${c?money(c.price):'—'}</b></div><div><span>BE</span><b>${c&&num(c.be)>0?money(c.be):'—'}</b></div><div><span>LIQ</span><b>${c?money(c.liq):'—'}</b></div><div><span>BE DIST.</span><b>${bd==null?'—':(bd>=0?'+':'')+bd.toFixed(1)+'%'}</b></div></div><div class="buffer-track"><i style="width:${Math.max(2,Math.min(100,(cr||0)*3.33))}%"></i></div><div class="cockpit-foot"><span>${watch} Bots &lt;30% Puffer</span><span>Short/Long ${e.coverage.toFixed(1)}%</span></div></section><section class="exposure-card"><div><span>LONG EXPOSURE</span><b>${money(e.longUsd)}</b></div><div><span>BTC HEDGE</span><b>${money(e.hedgeUsd)}</b></div><div><span>NET LONG</span><b>${money(e.netUsd)}</b></div><div><span>HEDGE COVERAGE</span><b class="tone-${e.coverage>=15?'safe':e.coverage>=7?'watch':'danger'}">${e.coverage.toFixed(1)}%</b></div></section>${manualStrip()}${okxStrip()}${riskCockpitV2()}${profitLockRadar()}<section class="action command-action"><span>NEXT ACTION</span><b>${action}</b><small>${actionReason||'Market sync'}</small></section><div class="quick-grid"><section><span>AKTIVE BOTS</span><b>${state.bots.length}</b><small>${new Set(state.bots.map(b=>b.symbol)).size} Assets</small></section><section><span>TP RADAR</span><b>${tp.length}</b><small>≤10% zum TP</small></section><section><span>AVG RISK</span><b>${rg.avgRisk.toFixed(1)}</b><small>Portfolio Score</small></section><section><span>BTC SETUP</span><b class="wait">${state.intel?state.intel.score+'/100':'SYNC'}</b><small>${state.intel?.status||'loading'}</small></section><section><span>WATCH PROFIT</span><b class="tone-watch">${sig.watch}</b><small>15m/1h Frühwarnung</small></section><section><span>PROFIT LOCK</span><b class="tone-watch">${sig.lock}</b><small>Teilgewinn prüfen</small></section><section><span>RISK REVIEW</span><b class="tone-danger">${sig.risk}</b><small>Struktur dreht</small></section><section><span>HOLD</span><b class="tone-safe">${sig.hold}</b><small>${sig.sync?'+'+sig.sync+' SYNC':'kein Lock-Signal'}</small></section></div><div class="section-title"><h2>RISK PRIORITY</h2><small>engster Liq-Puffer zuerst</small></div><div class="bots command-bots">${[...state.bots].sort((a,b)=>(risk(a)??999)-(risk(b)??999)).slice(0,4).map(botCard).join('')}</div>`}
...
async function binanceHistory(interval,limit,symbol){
 let rows=[],endTime=null;
 while(rows.length<limit){
  const take=Math.min(1000,limit-rows.length),u='https://api.binance.com/api/v3/klines?symbol='+symbol+'USDT&interval='+interval+'&limit='+take+(endTime!=null?'&endTime='+endTime:'');
