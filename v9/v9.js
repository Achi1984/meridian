...
async function sync(){try{const payload=await getJson('/api/private/dashboard'),d=payload?.data||payload,live=Array.isArray(d?.pionexRisk?.bots)?d.pionexRisk.bots.map(normalizeLive):[];state.bots=live.length?mergeReference(live):FALLBACK.map(ref=>({...ref,_liveMatched:false,_livePrice:false,_livePnl:false,_liveInvest:false,_source:'REFERENCE'}));if(!live.length)state.unmatchedLive=[];state.liveRows=live.length;const specificTs=parseTs(d?.pionexRisk?.updatedAt||d?.pionexRisk?.snapshotAt);state.botFeedUpdatedAt=specificTs||parseTs(d?.privateUpdatedAt);state.botFeedTimestampTrusted=!!specificTs;state.botFeedSource=specificTs?'PRIVATE PIONEX SNAPSHOT':'PRIVATE STATE · LEGACY TIMESTAMP';const matched=state.bots.filter(liveMatched).length;state.source=botFeedFresh()&&matched===state.bots.length&&matched?'FRESH':matched?'MIXED':'REFERENCE';state.market=String(d?.market?.regime||d?.btcRegime?.label||d?.regime?.label||'SYNC').toUpperCase();state.portfolio=portfolioModel(d);state.syncedAt=Date.now();state.error=null}catch(e){state.error=e.message;state.source='REFERENCE';state.botFeedTimestampTrusted=false}renderHeaderTruth();go(current)}
