...
function riskCockpitV2(){
 ...
 return '<section class="risk-v2"><div class="section-title"><h2>RISK COCKPIT V2</h2><small>Asset-Paare · Liq + Hedge + 15m/1h/4h + BTC-Regime</small></div><div class="risk-v2-grid">'+pairs.map(x=>'<article class="risk-v2-row"><div><strong>'+x.symbol+'</strong><span class="tag tone-'+x.tone+'">'+x.label+'</span></div><div><span>NET</span><b>'+money(x.netUsd)+'</b></div><div><span>HEDGE</span><b>'+x.hedgePct.toFixed(1)+'%</b></div><div><span>MIN LIQ</span><b>'+(x.minBuffer==null?'—':x.minBuffer.toFixed(1)+'%')+'</b></div><div><span>SIGNAL</span><b>'+x.profitAction+'</b></div><small>'+(x.reasons.join(' · ')||'kein neues Warnsignal')+'</small></article>').join('')+'</div></section>';
}
...
function signalSummary(){const out={hold:0,watch:0,lock:0,risk:0};for(const b of state.bots){const a=actionForSide(state.assetIntel[b.symbol],b.side||'LONG');if(a==='RISK REVIEW')out.risk++;else if(a==='PROFIT LOCK CANDIDATE')out.lock++;else if(a==='WATCH PROFIT')out.watch++;else out.hold++}return out}
