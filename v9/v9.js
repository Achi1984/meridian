...
function dataTruthCard(){const matched=state.bots.filter(liveMatched).length,pnl=state.bots.filter(livePnlAvailable).length,verified=Object.values(state.priceChecks||{}).filter(x=>x&&x.verified).length,pSource=state.portfolio?.pionexSource==='PRIVATE_ACCOUNT_TOTAL'?'LIVE ACCOUNT':'SNAPSHOT',mode=matched===0?'REFERENCE':matched===state.bots.length?'LIVE':'MIXED';return `<section class="data-truth"><div class="data-truth-head"><span>DATA TRUTH</span><b>${mode}</b></div><div class="truth-grid"><div><span>BOT MATCH</span><b>${matched}/${state.bots.length}</b></div><div><span>LIVE PNL</span><b>${pnl}</b></div><div><span>2-SOURCE PRICE</span><b>${verified}</b></div><div><span>PORTFOLIO</span><b>${pSource}</b></div></div><small>Referenzwerte dürfen keine Profit-Lock/Next-Action Entscheidung auslösen.</small></section>`}
