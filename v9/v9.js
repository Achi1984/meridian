...
function mergeReference(live){
  ...
  for(const m of matches){
    ...
    refs[m.ri]={...ref,...Object.fromEntries(Object.entries(x).filter(([,v])=>v!=null&&v!=='')),side:ref.side||'LONG',_liveMatched:true,_livePrice:num(x.price)>0,_livePnl:num(x.pnl)!=null||num(x.profitPct)!=null,_liveInvest:num(x.invest)!=null,_source:'LIVE_MATCH'};
...
function signalSummary(){const out={hold:0,watch:0,lock:0,risk:0,sync:0};for(const b of state.bots){if(!liveMatched(b)){out.sync++;continue}const a=actionForSide(state.assetIntel[b.symbol],b.side||'LONG');if(a==='RISK REVIEW')out.risk++;else if(a==='PROFIT LOCK CANDIDATE')out.lock++;else if(a==='WATCH PROFIT')out.watch++;else if(a==='HOLD')out.hold++;else out.sync++}return out}
...
function exposureModel(){const botNotional=b=>liveInvestAvailable(b)?(num(b.invest)||0)*(num(b.leverage)||1):null,knownPosition=b=>{const explicit=num(b.notionalUsd),margin=num(b.marginUsd),px=num(b.price)||num(b.entry),coin=num(b.sizeCoin)||num(b.investCoin),lev=num(b.leverage)||1;if(explicit!=null)return explicit;if(margin!=null)return margin*lev;if(coin!=null&&px>0)return coin*px*lev;return null};const confirmed=state.bots.filter(liveMatched),pionex=confirmed.map(b=>({...b,notional:botNotional(b)})).filter(b=>b.notional!=null),okx=(state.okxPositions||[]).map(b=>({...b,notional:knownPosition(b)})).filter(b=>b.notional!=null),manual=(state.pionexManual||[]).map(b=>({...b,notional:knownPosition(b)})).filter(b=>b.notional!=null),rows=[...pionex,...okx,...manual],unknownBots=confirmed.filter(b=>botNotional(b)==null).length,longUsd=rows.filter(b=>(b.side||'LONG')==='LONG').reduce((x,b)=>x+b.notional,0),shortUsd=rows.filter(b=>b.side==='SHORT').reduce((x,b)=>x+b.notional,0),grossUsd=longUsd+shortUsd,netUsd=longUsd-shortUsd,coverage=longUsd>0?shortUsd/longUsd*100:null;return{longUsd,shortUsd,hedgeUsd:shortUsd,grossUsd,netUsd,coverage,unknownBots,confirmedBots:confirmed.length,complete:confirmed.length>0&&unknownBots===0}}
