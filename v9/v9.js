...
function botMarketPrice(b){
 const feed=num(state.assetIntel&&b&&state.assetIntel[b.symbol]&&state.assetIntel[b.symbol].price),direct=num(b&&b.price);
 if(b&&b._livePrice&&direct>0)return direct;
...
function mergeReference(live){
  ...
  for(const m of matches){
    ...
    refs[m.ri]={...ref,...Object.fromEntries(Object.entries(x).filter(([,v])=>v!=null&&v!=='')),side:ref.side||'LONG',_liveMatched:true,_livePrice:num(x.price)>0};
...
function assetPairRisk(symbol){
 ...
 const notional=b=>{const p=botMarketPrice(b)||num(b.be),q=num(b.investCoin),usd=num(b.invest),lev=num(b.leverage)||1;return q!=null&&p>0?q*p*lev:(usd||0)*lev};
...
function riskCockpitV2(){
 ...
 return '<section class="risk-v2"><div class="section-title"><h2>RISK COCKPIT V2</h2><small>Asset-Paare · Liq + Hedge + 15m/1h/4h + BTC-Regime</small></div><div class="risk-v2-grid">'+pairs.map(x=>'<article class="risk-v2-row"><div><strong>'+x.symbol+'</strong><span class="tag tone-'+x.tone+'">'+x.label+'</span></div><div><span>NET</span><b>'+money(x.netUsd)+'</b></div><div><span>HEDGE</span><b>'+x.hedgePct.toFixed(1)+'%</b></div><div><span>MIN LIQ</span><b>'+(x.minBuffer==null?'—':x.minBuffer.toFixed(1)+'%')+'</b></div><div><span>MOMENTUM '+x.dominantSide+'</span><b class="tone-'+signalTone(x.profitAction)+'">'+x.profitAction+'</b></div><small>'+(x.reasons.join(' · ')||'kein neues Warnsignal')+'</small></article>').join('')+'</div></section>';
}
...
function assetExposureShare(symbol){const e=exposureModel(),x=state.bots.filter(b=>b.symbol===symbol).reduce((s,b)=>{const p=botMarketPrice(b)||num(b.be),coin=num(b.investCoin),usd=num(b.invest);return s+(coin!=null&&p>0?coin*p:(usd||0))},0);return e.longUsd>0?x/e.longUsd*100:0}
