/* MERIDIAN v8.0 R3 — customer-first CENTER command view. Presentation only; execution unchanged. */
(function(){
  'use strict';
  const VERSION='8.0';
  const BUILD='8.0-20260905-R3';
  ...
  function root(){return document.getElementById('view-live')||document.getElementById('view-dashboard')||document.getElementById('view-center')||document.querySelector('[data-view="live"],[data-view="dashboard"],[data-view="center"]')}
  function portfolio(){
  ...
  function risk(){
    const xs=Object.entries(window.MERIDIAN_PIONEX_SNAPSHOT?.activeBots||{}).map(([id,b])=>({id,buffer:Number(first(b?.pionexLiqBufferPct,b?.calculatedLiqBufferPct,b?.liqBufferPct))})).filter(x=>Number.isFinite(x.buffer));
    xs.sort((a,b)=>a.buffer-b.buffer);const c=xs[0]||null;
  ...
  function regime(){
    ...
    const v=first(r.currentRegime,r.marketRegime,r.selectedRegime,r.regime,r.state,window.DATA?.market?.regime);
  ...
  function ensure(){
    ...
    let host=document.getElementById('v8-center-summary');
    ...
    window.MERIDIAN_V8_CENTER_STATUS={version:VERSION,build:BUILD,portfolioUsd:p,regime:rg,risk:rk.label,criticalBot:rk.bot,bestOpportunity:opp||null,executionImpact:false,updatedAt:new Date().toISOString()};
  }
