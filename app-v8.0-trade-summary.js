/* MERIDIAN v8.0 R11 — customer-first TRADE summary. Presentation only; execution unchanged. */
(function(){
  'use strict';
  const VERSION='8.0';
  ...
  const root=()=>document.getElementById('view-daytrade')||document.getElementById('view-trade');
  ...
  function canonicalBots(){
    try{
      ...
      return rows.map(b=>({id:String(b?.id||b?.botId||b?.symbol||'BOT'),side:String(b?.side||b?.direction||'').toUpperCase()||'—',leverage:Number(b?.leverage||b?.leverageX||b?.x),buffer:Number(b?.buffer ?? b?.verifiedBuffer ?? b?.liquidationDistancePct ?? b?.liveEstimate),status:String(b?.guard?.label||b?.management?.phase||b?.unified?.phase||b?.status||'').toUpperCase()})).filter(b=>b.id);
    }catch(_e){return []}
  }
  function snapshotBots(){
    const active=window.MERIDIAN_PIONEX_SNAPSHOT?.activeBots;if(!active||typeof active!=='object')return [];
    return Object.entries(active).map(([id,b])=>({id,side:String(b?.side||b?.direction||'').toUpperCase()||'—',leverage:Number(b?.leverage||b?.leverageX||b?.x),buffer:Number(b?.pionexLiqBufferPct ?? b?.calculatedLiqBufferPct ?? b?.liqBufferPct ?? b?.liquidationDistancePct),status:String(b?.status||b?.riskState||'').toUpperCase()}));
  }
  function domBots(r){
  ...
  function state(buffer,status=''){
    ...
    const b=Number(buffer);if(b<8)return {label:'DANGER',tone:'red',target:'Puffer zuerst auf ≥8% bringen'};if(b<12)return {label:'WATCH',tone:'amber',target:'Puffer auf SAFE ≥12% erhöhen'};return {label:'SAFE',tone:'green',target:'Keine akute Liquidationsmaßnahme'};
  }
  function critical(xs){const unprotected=xs.filter(x=>!/PROTECTED/i.test(String(x.status||''))&&Number.isFinite(Number(x.buffer))).sort((a,b)=>Number(a.buffer)-Number(b.buffer));if(unprotected.length)return unprotected[0];return xs.filter(x=>Number.isFinite(Number(x.buffer))).sort((a,b)=>Number(a.buffer)-Number(b.buffer))[0]||xs[0]||null}
  ...
  function ensureSummary(r){
    if(!r||!['view-daytrade','view-trade'].includes(r.id))return;
    let host=r.querySelector(':scope > #v8-trade-summary');if(!host){host=document.createElement('section');host.id='v8-trade-summary';host.className='v8-customer-screen';r.prepend(host)}
    const xs=bots(r),c=critical(xs),s=state(c?.buffer,c?.status);
    ...
    window.MERIDIAN_V8_TRADE_STATUS={version:VERSION,build:BUILD,rootId:r.id,criticalBot:c?.id||null,buffer:Number.isFinite(Number(c?.buffer))?Number(c.buffer):null,state:s.label,botCount:xs.length,source:canonicalBots().length?'CANONICAL_BOT_STATES':snapshotBots().length?'PIONEX_SNAPSHOT':'TRADE_DOM',executionImpact:false,updatedAt:new Date().toISOString()};
  }
  function css(){if(document.getElementById('v8-trade-css'))return;const s=document.createElement('style');s.id='v8-trade-css';s.textContent=`
    #view-daytrade:not(.v8-trade-details-open)>.v8-trade-legacy,#view-trade:not(.v8-trade-details-open)>.v8-trade-legacy{display:none!important}
  ...
  let busy=false,timer=null;function apply(){if(busy)return;const r=root();if(!r)return;busy=true;try{css();ensureSummary(r)}finally{busy=false}}function schedule(){clearTimeout(timer);timer=setTimeout(apply,100)}
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();addEventListener('pageshow',()=>{stamp();apply()});document.addEventListener('visibilitychange',()=>{if(!document.hidden)apply()});addEventListener('meridian:v8-viewchange',e=>{if(e?.detail?.view==='trade')apply()});setInterval(()=>{if(!document.hidden)apply()},3000);
})();
