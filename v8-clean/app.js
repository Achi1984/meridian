import {loadCenter,loadDepot,loadTrade,loadPaper,hasReadToken,setReadToken} from './data.js?v=8.0-r36';
...
function paperHtml(x){
  ...
  const fundingTone=fc.lifecycle==='ACTIVE_PAPER'?'safe':fc.lifecycle==='STOPPED_REVIEW'?'danger':'muted';
  ...
  <section class="card"><div class="research-head"><div><span>BTC FUNDING CARRY V1</span><b class="tone-${fundingTone}">${fundingState}</b></div><div class="research-pnl tone-${fundingTone}">${fundingValue}</div></div><p class="muted">${fundingLine}</p></section>
