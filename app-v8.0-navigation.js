...
(function(){
  ...
  const ITEMS=[
    ...
    {key:'trade',label:'TRADE',patterns:[/^TRADE$/i,/^GRID$/i]},
    {key:'paper',label:'PAPER',patterns:[/^PAPER$/i]},
    {key:'more',label:'MORE',patterns:[]}
  ];
  ...
  function findLegacy(patterns){
    const xs=legacyCandidates();
  ...
  function markActive(key){
    document.querySelectorAll('#v8-bottom-nav button').forEach(b=>{const on=b.dataset.view===key;b.classList.toggle('active',on);b.setAttribute('aria-current',on?'page':'false')});
    window.MERIDIAN_V8_NAV_STATUS={version:VERSION,build:BUILD,active:key,items:ITEMS.map(x=>x.key),executionImpact:false,updatedAt:new Date().toISOString()};
  }
  function route(key){
    if(key==='more'){
      const more=document.getElementById('v8-more-open');
      if(more){more.click();markActive(inferActive());return true}
      return false;
    }
    const item=ITEMS.find(x=>x.key===key),legacy=findLegacy(item?.patterns||[]);
    if(legacy){legacy.click();setTimeout(()=>markActive(key),20);return true}
    const view=document.getElementById('view-'+key)||(key==='center'?(document.getElementById('view-live')||document.getElementById('view-dashboard')):null);
    if(view){
      document.querySelectorAll('[id^="view-"]').forEach(v=>{if(v!==view)v.classList.add('hidden')});view.classList.remove('hidden');markActive(key);return true;
    }
    return false;
  }
  ...
  function ensure(){
    css();
    let nav=document.getElementById('v8-bottom-nav');
    if(!nav){
      nav=document.createElement('nav');nav.id='v8-bottom-nav';nav.setAttribute('aria-label','MERIDIAN Hauptnavigation');
      nav.innerHTML=`<div class="v8-nav-inner">${ITEMS.map((x,i)=>`<button type="button" data-view="${x.key}" aria-label="${x.label}"><i aria-hidden="true">${i+1}</i><span>${x.label}</span></button>`).join('')}</div>`;
      document.body.appendChild(nav);
      nav.querySelectorAll('button').forEach(b=>b.onclick=()=>route(b.dataset.view));
    }
    hideLegacyBottom();
    markActive(inferActive());
  }
  function css(){
    ...
    const s=document.createElement('style');s.id='v8-navigation-css';s.textContent=`
      ...
      #v8-bottom-nav button.active{color:#55c8ff;background:#071a27;box-shadow:inset 0 0 0 1px rgba(85,200,255,.14)}
      #v8-bottom-nav button.active i{border-color:#228fc2;color:#9de2ff;background:#082335}
      #v8-bottom-nav button[data-view="more"] i{font-size:0}#v8-bottom-nav button[data-view="more"] i:after{content:'•••';font-size:7px;letter-spacing:0}
      html.v8-nav-ready #v8-center-summary,html.v8-nav-ready #v8-depot-summary,html.v8-nav-ready #v8-trade-summary,html.v8-nav-ready #v8-paper-summary{margin-bottom:18px!important}
      html.v8-nav-ready .v8-customer-screen{max-width:760px!important}
      @media(max-width:430px){
        html.v8-nav-ready #v8-customer-mode{margin:7px 10px 4px!important;padding:9px 10px!important;border-radius:13px!important;gap:4px!important}
        html.v8-nav-ready #v8-customer-mode b{font-size:10px!important}html.v8-nav-ready #v8-customer-mode span{font-size:7px!important;line-height:1.3!important}
        #v8-bottom-nav{padding-left:7px;padding-right:7px;padding-top:5px}.v8-nav-inner{gap:3px;border-radius:16px;padding:3px}
        #v8-bottom-nav button{min-height:46px;font-size:7px;letter-spacing:.04em;gap:3px;border-radius:11px}#v8-bottom-nav button i{width:16px;height:16px}
        html.v8-nav-ready .v8-customer-screen{padding-left:10px!important;padding-right:10px!important;margin-top:8px!important}
        html.v8-nav-ready .v8-center-hero,html.v8-nav-ready .v8-depot-head{padding:14px!important;border-radius:16px!important}
        html.v8-nav-ready .v8-center-card,html.v8-nav-ready .v8-depot-perf>div,html.v8-nav-ready .v8-depot-split>div,html.v8-nav-ready .v8-depot-top{border-radius:12px!important}
      }
      @media(max-width:360px){#v8-bottom-nav button{font-size:6.4px}.v8-nav-inner{gap:2px}}
