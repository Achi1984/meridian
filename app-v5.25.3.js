...
function canonicalBotState(b){
 ...
 const management=managementDecision({...b, liquidation, live, buffer, snapBuffer, verifiedBuffer, liveEstimate, verifyRequired, guard, rawGuard, protection, beProtected:protection.active, usesLive, stale});
 ...
 return {...b, liquidation, live, buffer, snapBuffer, verifiedBuffer, liveEstimate, verifyRequired, guard, rawGuard, protection, beProtected:protection.active, usesLive, stale, action, management, reason};
}
...
function decisionSSOT(){
 ...
 const queue=botStates.map(b=>{
   const m=b.management||managementDecision(b);
   return {
     type:'BOT', id:b.id, symbol:b.symbol, tone:m.tone||b.guard.cls, action:m.action,
     detail:`${fmt(b.buffer,1)}% Liq.-Puffer · ${b.leverage||'—'}x · ${m.phase}`
   };
 });
...
function lifecycleDecision(bot){
 ...
 else if(bot?.id==='BTC-S30' || (bot?.symbol==='BTC'&&side==='SHORT')){
   const m=btcShortRecoveryDecision(bot);
   action=m.action; tone=m.tone; reason=m.reason;
 }
 else if(b<Number(cfg.criticalBufferBelow||8)){ action='DEFEND / REDUCE'; tone='red'; reason='CRITICAL Liquidationspuffer überschreibt Momentum und Entry.'; }
 else if(b<Number(cfg.dangerBufferBelow||15)){ action='KEEP / NO ADD'; tone='red'; reason='DANGER: kein zusätzliches Hebelrisiko.'; }
