...
function continuousSummary(events,start,end,cfg=CLOUD_BT_CONFIG,full=null){
  ...
  for(let i=0;i<5;i++){
    ...
    const r=replayPrepared(events,from,to,cfg),baseline=compactStats(r.baseline),shadow=compactStats(r.shadow),challenger=compactStats(r.challenger),regime=compactStats(r.regime);
