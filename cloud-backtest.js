...
import { challengerV31Decision, CHALLENGER_V31_RULESET } from './challenger-v31.js';
...
function continuousSummary(events,start,end,cfg=CLOUD_BT_CONFIG,full=null){
  ...
  for(let i=0;i<5;i++){
    ...
    const r=replayPrepared(events,from,to,cfg),baseline=compactStats(r.baseline),shadow=compactStats(r.shadow),challenger=compactStats(r.challenger),challengerV3=compactStats(r.challengerV3),challengerV31=compactStats(r.challengerV31),regime=compactStats(r.regime);
    windows.push({i:i+1,from,to,...baseline,baseline,shadow,challenger,challengerV3,challengerV31,regime});
  }
