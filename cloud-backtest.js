...
function continuousSummary(events,start,end,cfg=CLOUD_BT_CONFIG,full=null){
  ...
  for(let i=0;i<5;i++){
    ...
    const r=replayPrepared(events,from,to,cfg),baseline=compactStats(r.baseline),shadow=compactStats(r.shadow),challenger=compactStats(r.challenger),regime=compactStats(r.regime),trendPullback=compactStats(r.trendPullback);
    windows.push({i:i+1,from,to,...baseline,baseline,shadow,challenger,regime,trendPullback});
  }
  ...
  return{
    ...
    variants:[
      ...
      variantStability('TREND PULLBACK V1','trendPullback',windows,whole.trendPullback)
    ]
  };
}
...
export async function runCloudBacktest({assets,days=90,end=Date.now(),onProgress=()=>{}}){
  ...
  const exitLabReplay=replayExitLabForLedgers({
    ledgers:{baseline:ledgers.baseline.tradeList,shadow:ledgers.shadow.tradeList,challenger:ledgers.challenger.tradeList,regime:ledgers.regime.tradeList,trendPullback:ledgers.trendPullback.tradeList},
    market,end,opts:{horizonDays:14,feeBps:cfg.feeBps,slippageBps:cfg.slippageBps}
  });
  const result={mode:'PORTFOLIO_V2',cloud:true,version:'7.49-EXIT-LAB-REPLAY-V1',researchEngine:'7.34-RESEARCH-V2',regimeModel:'7.38-REGIME-V1',trendPullbackModel:TREND_PULLBACK_V1_RULESET,method:'SHARED_MULTI_ASSET_TIMELINE_INDEPENDENT_LEDGERS',executionImpact:false,days,start,end,assets,config:cfg,summary:compactStats(ledgers.baseline),ledgers:{baseline:{...compactStats(ledgers.baseline),tradeList:ledgers.baseline.tradeList,equityCurve:ledgers.baseline.equityCurve,byAsset:assetBreakdown(ledgers.baseline.tradeList)},shadow:{...compactStats(ledgers.shadow),tradeList:ledgers.shadow.tradeList,equityCurve:ledgers.shadow.equityCurve,byAsset:assetBreakdown(ledgers.shadow.tradeList)},challenger:{...compactStats(ledgers.challenger),tradeList:ledgers.challenger.tradeList,equityCurve:ledgers.challenger.equityCurve,byAsset:assetBreakdown(ledgers.challenger.tradeList)},regime:{...compactStats(ledgers.regime),tradeList:ledgers.regime.tradeList,equityCurve:ledgers.regime.equityCurve,byAsset:assetBreakdown(ledgers.regime.tradeList)},trendPullback:{...compactStats(ledgers.trendPullback),tradeList:ledgers.trendPullback.tradeList,equityCurve:ledgers.trendPullback.equityCurve,byAsset:assetBreakdown(ledgers.trendPullback.tradeList)}},researchContinuous,exitLabReplay,generatedAt:new Date().toISOString()};
