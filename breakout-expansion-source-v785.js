// MERIDIAN v7.85 R2 — public Binance market source for Breakout/Expansion research.
...
async function fetchPage(symbol,interval,start,end,{fetchImpl,onRetry}){
  ...
  for(const endpoint of ENDPOINTS){
    for(let attempt=0;attempt<5;attempt++){
      ...
      try{
        const r=await fetchImpl(url,{headers:{'user-agent':'ACHI-MERIDIAN-BREAKOUT-EXPANSION/7.85'}});
...
export async function loadBreakoutMarket({assets,start,end,fetchImpl=globalThis.fetch,onProgress=()=>{}}={}){
