...
function commandCenter(){
 ...
 `<section class="card compact-now">
   ...
   <div class="compact-action">${x.riskOpen?(x.technicalOpen?'Entry-Plan prüfen.':'Auf Trigger warten.'):'NÄCHSTER SCHRITT: BTC-S30 auf ≥8% Puffer bringen.'}</div>
 </section>`+
 `<details class="card center-fold" data-detail-key="center-execution"><summary><span><b>EXECUTION & OPPORTUNITIES</b><small>${best?'#1 '+best.sym:'Watchlist'} · ${x.open?'actionable':'preview only'}</small></span><strong>ÖFFNEN</strong></summary><div class="fold-body">${moduleSafe('DUAL GATE',()=>dualGatePanel())}${moduleSafe('ENTRY INTELLIGENCE',()=>entryIntelligencePanel())}</div></details>`+
 `<details class="card center-fold" data-detail-key="center-risk"><summary><span><b>RISK & RECOVERY</b><small>${Number.isFinite(shortBuf)?'BTC-S30 '+fmt(shortBuf,2)+'%':'Portfolio'} · Risk ${risk.score}/100</small></span><strong>ÖFFNEN</strong></summary><div class="fold-body">${moduleSafe('LIVE RISK COCKPIT',()=>liveRiskCockpitPanel())}${moduleSafe('DYNAMIC RECOVERY',()=>compactRecoveryPanel())}${moduleSafe('RISK DETAILS',()=>centerAdvancedRiskDetails())}</div></details>`+
 `<details class="card center-fold" data-detail-key="center-portfolio"><summary><span><b>PORTFOLIO & MARKT</b><small>${m.label||DATA.market?.regime||'—'} · Bot $${fmt(botCapital,0)}</small></span><strong>ÖFFNEN</strong></summary><div class="fold-body">
   <div class="cc-grid"><div class="cc-tile"><span>MERIDIAN RISK</span><b>${risk.score}/100</b></div><div class="cc-tile"><span>MARKTREGIME</span><b class="cyan">${m.label||DATA.market?.regime||'—'}</b></div></div>
   ${portfolioChart()}
 </div></details>`+
 `<details class="card center-fold" data-detail-key="center-health"><summary><span><b>DATA HEALTH</b><small>Livefeed · Pionex · NADIR</small></span><strong>ÖFFNEN</strong></summary><div class="fold-body"><div class="compact-health">
