...
Comparator: frozen Hybrid Alpha v7.97 at `6905f7d2bc2386f0b5390acaff121718a52ce065`.  
...
- Correlation lookback is exactly 60 completed daily returns and will not be tuned after results.
- Effective-risk budget is exactly 1.00, reused from v7.101; no alternate-cap search.
...
- If required trailing correlation history is incomplete/non-finite, the bundle is left unchanged and flagged `CORRELATION_UNAVAILABLE`; no correlation is invented and the rejected flat-cap fallback is not used.
...
8. mean scale on scaled 90d bundles remains >= 0.60, preventing a pass by crushing nearly all exposure;
