...
let APP_RELEASE='5.26.0 · UNIFIED RISK ENGINE';
...
function commandCenter(){
 ...
 const gateText=x.open?'EXECUTION READY':x.technicalOpen?'SETUP READY · RISK BLOCK':'WAIT / BLOCKED';
 ...
 const riskAction=Number.isFinite(shortBuf)&&shortBuf<8?'BTC-S30 auf ≥8% Puffer bringen.':Number.isFinite(shortBuf)&&shortBuf<12?'BTC-S30 Richtung 12% SAFE stabilisieren.':'Risk Gate neu prüfen.';
 ...
 `<section class="card compact-now">
   ...
   <div class="compact-action">${x.riskOpen?(x.technicalOpen?'NÄCHSTER SCHRITT: Entry-Plan prüfen.':'NÄCHSTER SCHRITT: Auf Trigger warten.'):'NÄCHSTER SCHRITT: '+riskAction}</div>
 </section>`+
 `<details class="card center-fold center-focus" data-detail-key="center-opportunity"><summary><span><b>OPPORTUNITY</b><small>${best?'#1 '+best.sym:'Watchlist'} · ${x.open?'actionable':'preview only'}</small></span><strong>ÖFFNEN</strong></summary><div class="fold-body focus-body">
   ${oppRows||'<div class="muted">Noch keine priorisierte Opportunity.</div>'}
   <p class="footer-note">Watchlist ≠ Orderfreigabe. Details zu Setup/FIB bleiben im GRID-Tab.</p>
 </div></details>`+
 `<details class="card center-fold center-focus" data-detail-key="center-risk"><summary><span><b>RISK</b><small>${Number.isFinite(shortBuf)?'BTC-S30 '+fmt(shortBuf,2)+'%':'Portfolio'} · ${risk.score}/100</small></span><strong>ÖFFNEN</strong></summary><div class="fold-body focus-body">
   <div class="focus-grid"><div><span>BTC-S30</span><b class="${Number.isFinite(shortBuf)&&shortBuf<8?'red':'amber'}">${Number.isFinite(shortBuf)?fmt(shortBuf,2)+'%':'—'}</b></div><div><span>MERIDIAN RISK</span><b>${risk.score}/100</b></div><div><span>RISK GATE</span><b class="${x.riskOpen?'green':'red'}">${x.riskOpen?'OPEN':'BLOCKED'}</b></div><div><span>NEXT</span><b>${Number.isFinite(shortBuf)&&shortBuf<8?'8% RECOVERY':'12% SAFE'}</b></div></div>
   <div class="focus-callout">${riskAction}</div>
   <p class="footer-note">Tiefe Position-/Recovery-Analyse bleibt im GRID-Tab.</p>
 </div></details>`+
 `<details class="card center-fold center-focus" data-detail-key="center-portfolio"><summary><span><b>PORTFOLIO</b><small>${m.label||DATA.market?.regime||'—'} · Bot $${fmt(botCapital,0)}</small></span><strong>ÖFFNEN</strong></summary><div class="fold-body focus-body">
   ...
   <p class="footer-note">Allokation, Positionen und Verlauf sind im DEPOT; Regime-Details im MARKT-Tab.</p>
 </div></details>`+
 `<details class="card center-fold center-focus" data-detail-key="center-health"><summary><span><b>DATA HEALTH</b><small>Livefeed · Pionex · NADIR</small></span><strong>ÖFFNEN</strong></summary><div class="fold-body focus-body"><div class="compact-health">
  <div class="health-row"><span><i class="dot live"></i>BTC Livefeed</span><b class="green">${fh.status==='LIVE'?'LIVE':'FALLBACK'}</b></div>
...
function dayTrade(){
 ...
 return card(`<div class="eyebrow">DAY-TRADE 2.3 · TECHNICAL GATE</div><div class="forecast-main">${d.entryAllowed?'TECHNICAL READY':'TECHNICAL BLOCKED'}</div><div class="sub">${d.entryAllowed?'Setup technisch bestätigt · Portfolio-Gate separat prüfen.':(d.decisionNote||'Bias ≠ Ausführung')}</div><div class="row"><b>GATE SCORE</b><b class="${d.entryAllowed?'green':'amber'}">${d.gateScore}/100</b></div><div class="bar"><i style="width:${d.gateScore}%"></i></div>`)+
...
function gridView(){
 ...
 return card(`<div class="eyebrow">GRID COMMANDER 3.7 ${snapshotBadge('UNIFIED RISK SSOT')}</div><div class="forecast-main">HEDGE FIRST</div><div class="sub">Unified Risk State · Liquidation Guard · Entry Readiness</div>
 <div class="grid2">${metric('FUTURES RISK',futuresRisk,critical?'red':danger?'amber':'green')}${metric('BTC BOTS',bots.filter(b=>b.symbol==='BTC').length+' · '+(bots.some(b=>b.symbol==='BTC'&&String(b.side).toUpperCase()==='LONG')?'LONG':'')+(bots.some(b=>b.symbol==='BTC'&&String(b.side).toUpperCase()==='SHORT')?' + SHORT':''),'cyan')}${metric('CRITICAL',critical,critical?'red':'green')}${metric('DANGER/RECOVERY',danger,danger?'amber':'green')}</div><p class="footer-note">SSOT: ACTIVE Pionex Bots → Canonical Buffer → Unified Risk State 2.0. Snapshot/Stale blockiert ADD, erfindet aber keinen neuen Buffer.</p>`)+moduleSafe('POSITION LIFECYCLE',()=>positionLifecyclePanel())+
 ...
 moduleSafe('BOT PRIORITY QUEUE',()=>card(`<div class="section-title">BOT PRIORITY QUEUE</div>${order.map(commanderBot).join('')}<p class="footer-note">Unified Risk State 2.0 steuert die Priorität; Details per Tap.</p>`))+
 moduleSafe('MULTI-ASSET SL MATRIX',()=>card(`<div class="section-title">MULTI-ASSET SL MATRIX <span class="tag cyan">MODEL</span></div>
 ${['SOL','ETH','XRP','HBAR','PEPE'].map(s=>multiAssetRiskRow(s)).join('')}
 <p class="footer-note">SL wird strukturell aus Range-Support + Volatilitätspuffer abgeleitet. Entry/SL/TP/R:R sind Modellwerte und keine automatische Order.</p>`))+moduleSafe('CONFLUENCE SUMMARY',()=>confluenceSummaryPanel())+
card(`<div class="eyebrow">FIB GRID ENGINE 1.3 ${liveBadge('LIVE')}</div><div class="forecast-main">ENTRY READINESS</div><button class="mini-grid-btn" onclick="refreshGridEngine(true)">4H SWINGS JETZT NEU LADEN</button><p class="footer-note">Setup Quality bewertet die Struktur. Entry Readiness bewertet, ob der Einstieg JETZT sinnvoll ist.</p>`)+
 ...
 moduleSafe('OPPORTUNITY SCANNER',()=>card(`<div class="section-title">OPPORTUNITY SCANNER</div>${['SOL','ETH','PEPE'].map(scannerCard).join('')||'<p class="footer-note">Scanner lädt 4H-Daten …</p>'}<p class="footer-note">Keine automatische Bot-Ausführung.</p>`));
}
