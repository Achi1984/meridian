...
function modelRows(replay){
  ...
  for(const [bot,ledger] of Object.entries(safe(replay?.ledgers))){
    for(const [model,m] of Object.entries(safe(ledger?.aggregate?.models))){
      rows.push({
        bot,model,trades:m.trades,totalR:round(m.totalR),avgR:round(m.avgR),medianR:round(m.medianR),
        winRateR:round(m.winRateR,1),avgGivebackR:round(m.avgGivebackR),tp1Rate:round(m.tp1Rate,1),
        tp1ToBeStopRate:round(m.tp1ToBeStopRate,1),tp2TouchAfterTp1Rate:round(m.tp2TouchAfterTp1Rate,1),
        deltaVsCurrent:round(m.deltaTotalRvsCurrent)
      });
    }
  }
  return rows;
...
function mdTable(rows){
  const h='| Bot | Modell | Trades | Total R | Avg R | Median R | R-WR | Avg Giveback R | TP1 | TP1→BE Stop | TP2 nach TP1 | ΔR vs A |\n|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|';
