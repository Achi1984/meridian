...
const START=Date.parse(C.evaluationStart),FUNDING_START=START,SPLIT=Date.parse(C.splitAt),END=Date.parse(C.evaluationEnd);
...
async function fetchZipCsv(url,tmpDir){
  const r=await fetch(url,{headers:{'user-agent':'MERIDIAN-BASIS-CONVERGENCE-V1/1.0'}});
...
const tmpDir=await fs.mkdtemp(path.join(os.tmpdir(),'meridian-basis-convergence-'));
try{
  const data={};
  ...
  let basket=null,cooldownUntil=START,realized=0,portfolioEquity=C.referenceCapital,peakEquity=portfolioEquity,maxMarkedDrawdown=0;
  const cycles=[],entries=[];

  const markBasket=(b,t)=>{
    const d=data[b.symbol],spot=d.spotAt.get(t),perp=d.perpAt.get(t);
    if(!spot||!perp)return null;
    const x=exitExecution(spot.o,perp.o);
    const spotPnl=b.qty*(x.spotBid-b.spotEntry);
    const perpPnl=b.qty*(b.perpEntry-x.perpAsk);
    const exitFees=b.qty*x.spotBid*C.spotFeeRate+b.qty*x.perpAsk*C.perpFeeRate;
    const net=spotPnl+perpPnl+b.fundingIncome-b.entryFees-exitFees;
    return{...x,spotPnl,perpPnl,exitFees,netPnl:net};
  };
  ...
  for(let t=START;t<END;t+=H4){
    ...
    if(basket){
      const rows=data[basket.symbol].fundingRows;
      while(basket.appliedFundingPtr<ptr[basket.symbol]){
        const f=rows[basket.appliedFundingPtr++];
        if(f.fundingTime>basket.openedAt&&f.fundingTime<=t&&Number.isFinite(f.markPrice)){
          basket.fundingIncome+=basket.qty*f.markPrice*f.fundingRate;
          basket.fundingCount++;
        }
      }
      const mark=markBasket(basket,t);
      if(mark){
        const held=t-basket.openedAt;
        let reason=null;
        if(mark.netPnl<=-C.maxMarkedLossUsd)reason='MARKED_LOSS';
        else if(mark.basisPct>=basket.entryBasisPct+C.maxBasisWideningPct)reason='BASIS_WIDENING';
        else if(mark.basisPct<=C.exitBasisPct)reason='BASIS_CONVERGED';
        else if(held>=C.maxHoldMs)reason='MAX_HOLD';

        if(reason){
          const row={
            symbol:basket.symbol,openedAt:new Date(basket.openedAt).toISOString(),closedAt:new Date(t).toISOString(),openedAtMs:basket.openedAt,closedAtMs:t,
            holdDays:round(held/DAY,3),exitReason:reason,qty:basket.qty,entryBasisPct:round(basket.entryBasisPct,5),exitBasisPct:round(mark.basisPct,5),
            fundingIncome:basket.fundingIncome,fundingCount:basket.fundingCount,spotPnl:mark.spotPnl,perpPnl:mark.perpPnl,
            entryFees:basket.entryFees,exitFees:mark.exitFees,totalFees:basket.entryFees+mark.exitFees,netPnl:mark.netPnl
          };
          cycles.push(row);realized+=row.netPnl;basket=null;cooldownUntil=t+C.cooldownMs;
        }
      }
    }

    if(!basket&&t>=cooldownUntil){
      const candidates=[];
      for(const symbol of C.symbols){
        const d=data[symbol],spot=d.spotAt.get(t),perp=d.perpAt.get(t);
        if(!spot||!perp)continue;
        const e=entryExecution(spot.o,perp.o);
        candidates.push({symbol,...e,spotOpen:spot.o,perpOpen:perp.o});
      }
      const selected=selectBasisCandidate(candidates);
      if(selected){
        const qty=C.spotNotional/selected.spotAsk;
        const entryFees=qty*selected.spotAsk*C.spotFeeRate+qty*selected.perpBid*C.perpFeeRate;
        basket={
          symbol:selected.symbol,openedAt:t,spotEntry:selected.spotAsk,perpEntry:selected.perpBid,entryBasisPct:selected.basisPct,
          qty,entryFees,fundingIncome:0,fundingCount:0,appliedFundingPtr:ptr[selected.symbol]
        };
        entries.push({at:new Date(t).toISOString(),symbol:selected.symbol,entryBasisPct:round(selected.basisPct,5),qty:round(qty,8)});
      }
    }

    const activeMark=basket?markBasket(basket,t):null;
    portfolioEquity=C.referenceCapital+realized+Number(activeMark?.netPnl||0);
  ...
  const stressed=cycles.map(x=>x.netPnl-C.extraStressUsdPerBasket),stressedNet=stressed.reduce((a,b)=>a+b,0),stressedPf=pf(stressed);
  const participatingAssets=C.symbols.filter(s=>(byAsset[s]?.cycles||0)>=4&&(byAsset[s]?.netPnl||0)>0).length;
  const years=Object.values(byYear),positiveYears=years.filter(x=>(x.netPnl||0)>=0).length;

  const dataQuality={};
  for(const symbol of C.symbols){
    const d=data[symbol],spotCov=coverage(d.spot.rows,START,END),perpCov=coverage(d.perp.rows,START,END),fundGap=maxFundingGapHours(d.fundingRows,START,END);
    const fundingComplete=d.fundingRows.length>0&&d.fundingRows[0].fundingTime<=START+8*HOUR&&d.fundingRows.at(-1).fundingTime>=END-16*HOUR&&fundGap<=12&&d.funding.dailyMissing.length===0;
    dataQuality[symbol]={spot:spotCov,perp:perpCov,fundingPeriods:d.fundingRows.length,maxFundingGapHours:round(fundGap,2),fundingComplete,spotRepair:d.spot.repair,perpRepair:d.perp.repair,dailyFundingMissing:d.funding.dailyMissing};
  }
  const dataAdequacy=Object.values(dataQuality).every(x=>x.spot.complete&&x.perp.complete&&x.fundingComplete);

  const gates={
    sample:overall.cycles>=20,
    aggregateNet:(overall.netPnl||0)>0,
    profitFactor:(overall.profitFactor||0)>=1.25,
    profitableShare:(overall.profitableShare||0)>=.60,
    markedDrawdown:maxMarkedDrawdown<=250,
    primary:primary.cycles>=7&&(primary.netPnl||0)>0&&(primary.profitFactor||0)>=1.10,
    secondary:secondary.cycles>=7&&(secondary.netPnl||0)>0&&(secondary.profitFactor||0)>=1.10,
    assetBreadth:participatingAssets>=2,
  ...
  const out={
    schemaVersion:'BASIS-CONVERGENCE-V1-EVIDENCE',generatedAt:new Date().toISOString(),researchOnly:true,executionImpact:false,
    predeclaredDesign:'research/basis-convergence-v1-design.md',source:'BINANCE_VISION_OFFICIAL_ARCHIVES',
    window:{evaluationStart:C.evaluationStart,splitAt:C.splitAt,evaluationEnd:C.evaluationEnd},
    overall,primary,secondary,byAsset,byYear,
    markedPortfolio:{endingEquity:round(portfolioEquity,2),peakEquity:round(peakEquity,2),maxMarkedDrawdown:round(maxMarkedDrawdown,2),maxMarkedDrawdownPct:round(maxMarkedDrawdown/C.referenceCapital*100,3)},
    assetPositivePnlShare,stressedNetPnl:round(stressedNet,2),stressedProfitFactor:round(stressedPf,2),participatingAssets,
    entries:entries.length,exitReasons:Object.fromEntries(Object.entries(cycles.reduce((m,x)=>(m[x.exitReason]=(m[x.exitReason]||0)+1,m),{})).sort()),
    gates,pass:Object.values(gates).every(Boolean),dataQuality,
    openTail:basket?{symbol:basket.symbol,openedAt:new Date(basket.openedAt).toISOString(),entryBasisPct:basket.entryBasisPct,fundingIncome:basket.fundingIncome}:null,
    cycles
  };
  await fs.mkdir('artifacts',{recursive:true});
  await fs.writeFile('artifacts/basis-convergence-v1-evidence.json',JSON.stringify(out,null,2));
  console.log(JSON.stringify({overall,primary,secondary,byAsset,byYear,markedPortfolio:out.markedPortfolio,assetPositivePnlShare,stressedNetPnl:out.stressedNetPnl,stressedProfitFactor:out.stressedProfitFactor,participatingAssets,entries:out.entries,exitReasons:out.exitReasons,gates,pass:out.pass,dataQuality,cycles:cycles.map(x=>({symbol:x.symbol,openedAt:x.openedAt,closedAt:x.closedAt,entryBasisPct:x.entryBasisPct,exitBasisPct:x.exitBasisPct,netPnl:round(x.netPnl,2),fundingIncome:round(x.fundingIncome,2),exitReason:x.exitReason}))},null,2));
}finally{
