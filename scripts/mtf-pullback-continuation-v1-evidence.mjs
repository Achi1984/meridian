...
import {MTF_PULLBACK_CONTINUATION_V1 as C,runMtfPullbackContinuationV1} from '../mtf-pullback-continuation-v1.js';
...
async function getJson(url,attempt=0){
  const r=await fetch(url,{headers:{'user-agent':'MERIDIAN-MTF-PULLBACK-V1/1.0','accept':'application/json'}});
...
function coverage(bars,start,end){const expected=Math.floor((end-start)/H4),actual=bars.filter(x=>x.t>=start&&x.t<end).length;return {expected4h:expected,actual4h:actual,complete:actual===expected};}
...
for(const symbol of C.symbols){
  ...
  collected[symbol]={rawCalls:raw.calls,bars,
    ...
    primary:runMtfPullbackContinuationV1(primaryBars,{symbol,entryStart:PRIMARY_START,entryEnd:PRIMARY_END}),
    secondary:runMtfPullbackContinuationV1(secondaryBars,{symbol,entryStart:SECONDARY_START,entryEnd:SECONDARY_END})};
}
...
const out={
  schemaVersion:'MTF-PULLBACK-CONTINUATION-V1-HISTORICAL-EVIDENCE',generatedAt:new Date().toISOString(),researchOnly:true,executionImpact:false,
  predeclaredDesign:'research/mtf-pullback-continuation-v1-design.md',config:C,source:'OKX_PUBLIC_SPOT_CONFIRMED_4H',
...
await fs.writeFile('artifacts/mtf-pullback-continuation-v1-evidence.json',JSON.stringify(out,null,2));
