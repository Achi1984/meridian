...
async function fetchZipCsv(url,tmpDir){
  const r=await fetch(url,{headers:{'user-agent':'MERIDIAN-CROSS-ASSET-CARRY-RISK-BUDGET-V1/1.0'}});
...
const tmpDir=await fs.mkdtemp(path.join(os.tmpdir(),'meridian-carry-risk-budget-'));
try{
  const data={};
  ...
  const cycles=[];
  let realized=0,portfolioEquity=C.portfolioReferenceCapital,peakEquity=portfolioEquity,maxMarkedDrawdown=0,maxSimultaneousBaskets=0,admissionPointer=C.initialPointer;
  ...
  for(let t=START;t<END;t+=H4){
    ...
    let slots=C.maxConcurrent-C.symbols.filter(s=>states[s]).length;
    if(slots>0){
      const order=cyclicAdmissionOrder(admissionPointer);
      for(const symbol of order){
        ...
        state=markFundingCarryV2(state,{spotBid:spot.o,perpAsk:perp.o},t);
        states[symbol]=state;entryEligibility[symbol]=eligibility;slots--;
        admissionPointer=nextAdmissionPointer(symbol);
      }
    }
  ...
  const gates={
    ...
    dataAdequacy,
    concurrency:maxSimultaneousBaskets<=C.maxConcurrent
  };
  ...
  const out={
    schemaVersion:'CROSS-ASSET-FUNDING-CARRY-RISK-BUDGET-V1-EVIDENCE',generatedAt:new Date().toISOString(),researchOnly:true,executionImpact:false,
    predeclaredDesign:'research/cross-asset-funding-carry-risk-budget-v1-design.md',source:'BINANCE_VISION_OFFICIAL_ARCHIVES',
  ...
  await fs.writeFile('artifacts/cross-asset-funding-carry-risk-budget-v1-evidence.json',JSON.stringify(out,null,2));
