...
import {
  CROSS_ASSET_FUNDING_CARRY_ROTATION_V1 as C,
  selectFundingCarryAsset
} from '../cross-asset-funding-carry-rotation-v1.js';
...
async function fetchZipCsv(url,tmpDir){
  const r=await fetch(url,{headers:{'user-agent':'MERIDIAN-CROSS-ASSET-CARRY-V1/1.0'}});
...
const tmpDir=await fs.mkdtemp(path.join(os.tmpdir(),'meridian-cross-carry-'));
try{
  const data={};
  ...
  let state=null,activeSymbol=null,cooldownUntil=START,activeEntryEligibility=null;
    ...
    if(state&&activeSymbol){
      ...
      if(spot&&perp){
        ...
        if(reason){
          ...
          const done=state.closedCycles.at(-1);
          cycles.push({
            symbol:activeSymbol,openedAt:done.openedAt,closedAt:done.closedAt,openedAtMs:Date.parse(done.openedAt),closedAtMs:Date.parse(done.closedAt),
            holdDays:round((Date.parse(done.closedAt)-Date.parse(done.openedAt))/DAY,3),exitReason:done.exitReason,
            entryCoverage:activeEntryEligibility?.grossCostCoverage??null,entryPositiveShare:activeEntryEligibility?.positiveShare??null,entryBasisPct:done.entryBasisPct,
            fundingIncome:Number(done.fundingIncome||0),basisPnl:Number(done.basisPnl||0),totalEstimatedCosts:Number(done.totalEstimatedCosts||0),netPnl:Number(done.realizedPnl??done.netPnl??0)
          });
          state=null;activeSymbol=null;activeEntryEligibility=null;cooldownUntil=t+C.cooldownMs;
        }
      }
    }

    if(!state&&t>=cooldownUntil){
      const candidates=[];
      for(const symbol of C.symbols){
      ...
      const selected=selectFundingCarryAsset(candidates);
      if(selected){
        state=newFundingCarryV2State(t,selected.cfg);
        state=openFundingCarryV2(state,{spotAsk:selected.spot.o,perpBid:selected.perp.o},selected.eligibility,t);
        state=markFundingCarryV2(state,{spotBid:selected.spot.o,perpAsk:selected.perp.o},t);
        activeSymbol=selected.symbol;activeEntryEligibility=selected.eligibility;
        selections.push({at:new Date(t).toISOString(),symbol:selected.symbol,grossCostCoverage:selected.eligibility.grossCostCoverage,positiveShare:selected.eligibility.positiveShare,basisPct:selected.eligibility.executableEntryBasisPct});
      }
    }
  }
  ...
  const assetCycleShare=Object.fromEntries(C.symbols.map(s=>[s,cycles.length?round(cycles.filter(x=>x.symbol===s).length/cycles.length,4):0]));
  const assetPositiveShare=Object.fromEntries(C.symbols.map(s=>[s,positiveSum>0?round(positive.filter(x=>x.symbol===s).reduce((a,x)=>a+x.netPnl,0)/positiveSum,4):0]));
  ...
  const participatingAssets=C.symbols.filter(s=>(byAsset[s]?.cycles||0)>=2&&(byAsset[s]?.netPnl||0)>0).length;
  ...
  const gates={
    ...
    maxDrawdown:(overall.maxDrawdown??Infinity)<=300,
    primary:primary.cycles>=5&&(primary.netPnl||0)>0&&(primary.profitFactor||0)>=1.25,
    secondary:secondary.cycles>=5&&(secondary.netPnl||0)>0&&(secondary.profitFactor||0)>=1.25,
    assetBreadth:participatingAssets>=2,
    cycleConcentration:Math.max(...Object.values(assetCycleShare))<=.70,
    pnlConcentration:Math.max(...Object.values(assetPositiveShare))<=.70,
  ...
  const out={
    schemaVersion:'CROSS-ASSET-FUNDING-CARRY-ROTATION-V1-EVIDENCE',generatedAt:new Date().toISOString(),researchOnly:true,executionImpact:false,
    predeclaredDesign:'research/cross-asset-funding-carry-rotation-v1-design.md',source:'BINANCE_VISION_OFFICIAL_ARCHIVES',
    ...
    assetCycleShare,assetPositivePnlShare:assetPositiveShare,participatingAssets,
    selectionCount:selections.length,selectionPreview:selections.slice(0,20),exitReasons:Object.fromEntries(Object.entries(cycles.reduce((m,x)=>(m[x.exitReason]=(m[x.exitReason]||0)+1,m),{})).sort()),
    gates,pass:Object.values(gates).every(Boolean),dataQuality,
    openTail:state&&activeSymbol?{symbol:activeSymbol,openedAt:state.basket?.openedAt,netPnl:state.basket?.netPnl,fundingIncome:state.basket?.fundingIncome}:null,
    cycles
  };
  await fs.mkdir('artifacts',{recursive:true});
  await fs.writeFile('artifacts/cross-asset-funding-carry-rotation-v1-evidence.json',JSON.stringify(out,null,2));
  console.log(JSON.stringify({overall,primary,secondary,byAsset,byYear,fundingMinusModeledCosts:out.fundingMinusModeledCosts,stressedNetPnl:out.stressedNetPnl,stressedProfitFactor:out.stressedProfitFactor,assetCycleShare,assetPositivePnlShare:assetPositiveShare,participatingAssets,gates,pass:out.pass,dataQuality,cycles:cycles.map(x=>({symbol:x.symbol,openedAt:x.openedAt,closedAt:x.closedAt,netPnl:round(x.netPnl,2),fundingIncome:round(x.fundingIncome,2),basisPnl:round(x.basisPnl,2),exitReason:x.exitReason,entryCoverage:x.entryCoverage}))},null,2));
}finally{
