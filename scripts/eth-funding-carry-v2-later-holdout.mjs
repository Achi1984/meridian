...
const C=Object.freeze({...BASE_CONFIG,symbol:SYMBOL});
const AUDIT_START=Date.parse('2024-01-01T00:00:00.000Z');
const AUDIT_END=Date.parse('2026-06-01T00:00:00.000Z');
const FUNDING_START=Date.parse('2023-12-01T00:00:00.000Z');
...
async function fetchZipCsv(url,tmpDir){
  const r=await fetch(url,{headers:{'user-agent':'MERIDIAN-ETH-FUNDING-CARRY-V2-HOLDOUT/1.0'}});
  if(r.status===404)return null;
  if(!r.ok)throw new Error(`HTTP ${r.status} ${url}`);
  const buf=Buffer.from(await r.arrayBuffer());
  const p=path.join(tmpDir,`eth-v2-${Math.random().toString(36).slice(2)}.zip`);
...
function monthUrl(kind,ym){
  if(kind==='spot')return `${VISION}/spot/monthly/klines/${SYMBOL}/4h/${SYMBOL}-4h-${ym}.zip`;
  if(kind==='swap')return `${VISION}/futures/um/monthly/klines/${SYMBOL}/4h/${SYMBOL}-4h-${ym}.zip`;
  return `${VISION}/futures/um/monthly/fundingRate/${SYMBOL}/${SYMBOL}-fundingRate-${ym}.zip`;
}
function dayUrl(kind,day){
  if(kind==='spot')return `${VISION}/spot/daily/klines/${SYMBOL}/4h/${SYMBOL}-4h-${day}.zip`;
...
const tmpDir=await fs.mkdtemp(path.join(os.tmpdir(),'meridian-eth-carry-v2-holdout-'));
try{
  console.log('loading Binance Vision ETHUSDT spot/perpetual/funding archives');
  ...
  const gates={
    ...
    maxDrawdown:dd<=250,
    calendarBreadth:yearsWithCycles.length>=2&&positiveYears===yearsWithCycles.length,
  ...
  const out={
    schemaVersion:'ETH-FUNDING-CARRY-V2-LATER-HOLDOUT',generatedAt:new Date().toISOString(),researchOnly:true,executionImpact:false,
    productionRuleset:'ETH-FUNDING-CARRY-V2-TRANSFERRED-BTC-V2-POLICY',predeclaredDesign:'research/eth-funding-carry-v2-later-holdout-design.md',
    ...
    productionConfig:C,summary,yearStats,gates,holdoutPass:Object.values(gates).every(Boolean),
  ...
  await fs.writeFile('artifacts/eth-funding-carry-v2-later-holdout.json',JSON.stringify(out,null,2));
  console.log(JSON.stringify({summary:out.summary,yearStats:out.yearStats,gates:out.gates,holdoutPass:out.holdoutPass,cycles:out.cycles,data:out.data},null,2));
