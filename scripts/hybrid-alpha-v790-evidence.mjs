...
const END=Date.now();
const START=END-MAX_DAYS*86400000-5*86400000;
...
function buildSamples(data,symbol,horizonBars){
  ...
  const closes=xs.map(x=>x.c),e20=ema(closes,20),e50=ema(closes,50),e200=ema(closes,200),a14=atr(xs,14);
  ...
  for(let i=200;i<xs.length-horizonBars;i++){
    ...
    out.push({timestamp:new Date(xs[i].t).toISOString(),symbol,forwardR,features:{regime:regimeOf(btc,bi,b20,b50,b200,ba14),trend,momentum,relativeStrength,meanReversion,orderFlow,volatilityRatio,liquidityQuality,reversalRisk:clamp(Math.abs(meanReversion)*Math.max(0,trend*meanReversion)*-1,0,1)}});
  }
  return out;
}
...
const evidence={schemaVersion:'7.90-HYBRID-EVIDENCE-V1',generatedAt:new Date().toISOString(),researchOnly:true,executionImpact:false,source:'BINANCE_FUTURES_PUBLIC_15M',symbols:SYMBOLS,windows:WINDOWS,horizons:{},notes:['No funding/carry input in this first public-candle evidence pass.','orderFlow uses Binance kline taker-buy quote-volume imbalance as a bar-level proxy, not full order-book OFI.','forwardR is future return divided by decision-time ATR14 percent; it is a normalized research outcome, not Baseline TP/SL R.']};
