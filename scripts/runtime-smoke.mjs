...
async function request(url,options={}){
  ...
  try{return await fetch(url,{cache:'no-store',redirect:'manual',...options,signal:c.signal})}finally{clearTimeout(timer)}
}
...
async function smoke(){
  ...
  if(!loader.includes('app-v7.32-legacy.js'))fail('Canonical loader missing base renderer');
  if(loader.indexOf('app-v7.32-legacy.js')>loader.indexOf('app-v7.33-hardening.js'))fail('Base renderer loads after hardening');
  ...
  const renderer=await text(`${PAGES_BASE}/app-v7.32-legacy.js?smoke=${nonce}`);
  if(!(renderer.includes('function renderAll')||renderer.includes('renderAll=')))fail('Deployed base renderer missing renderAll');
  ...
  for(const [route,hash] of [['center.html','#center'],['depot.html','#portfolio'],['market.html','#market'],['trade.html','#daytrade'],['forecast.html','#forecast']]){
    const shim=await text(`${PAGES_BASE}/${route}?smoke=${nonce}`);if(!shim.includes(`index.html${hash}`))fail(`${route} is still a stale standalone snapshot`);
  }
  ...
  return {ok:true,version:EXPECTED_VERSION,buildId:EXPECTED_BUILD,pages:true,renderer:true,approvedLogo:true,sixTabNav:true,legacyRoutes:true,gateway:true,privateData:true,anonymousProtectedStatus:protectedResponse.status,...sha,checkedAt:new Date().toISOString()};
}
let last;
