...
async function smoke(){
  ...
  if(!index.includes('function depot(){'))fail('Canonical index missing inline depot renderer');
  ...
  if(!polish.includes('.bottom .inner.five-tabs'))fail('Deployed mobile shell does not override legacy five-tab specificity');
  const logo=await request(`${PAGES_BASE}/assets/achi-meridian-topbar.webp?smoke=${nonce}`);
  if(!logo.ok)fail(`Approved logo asset HTTP ${logo.status}`);
  ...
  for(const [route,view] of [['center.html','center'],['depot.html','portfolio'],['market.html','market'],['trade.html','daytrade'],['forecast.html','forecast']]){
    const shim=await text(`${PAGES_BASE}/${route}?smoke=${nonce}`);
    if(!shim.includes(`route=${view}`)||!shim.includes(`#${view}`))fail(`${route} does not route to canonical index`);
    if(!shim.includes('background:#03070c'))fail(`${route} can still show a white boot screen`);
    if(!shim.includes('getRegistrations')||!shim.includes('caches.keys'))fail(`${route} does not clear stale PWA state`);
  }
  ...
  return {ok:true,version:EXPECTED_VERSION,buildId:EXPECTED_BUILD,pages:true,inlineRenderer:true,approvedLogo:true,sixTabNav:true,pwaRecovery:true,legacyRoutes:true,gateway:true,privateData:true,anonymousProtectedStatus:protectedResponse.status,...sha,checkedAt:new Date().toISOString()};
}
let last;
