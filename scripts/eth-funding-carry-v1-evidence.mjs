...
const AUDIT_START=Date.parse('2020-02-01T00:00:00.000Z');
const AUDIT_END=Date.parse('2024-01-01T00:00:00.000Z');
const FUNDING_START=Date.parse('2020-01-01T00:00:00.000Z');
...
const tmpDir=await fs.mkdtemp(path.join(os.tmpdir(),'meridian-eth-carry-v1-'));
try{
  ...
  const out={
    schemaVersion:'ETH-FUNDING-CARRY-V1-HISTORICAL-VALIDATION',generatedAt:new Date().toISOString(),researchOnly:true,executionImpact:false,
    productionRuleset:'ETH-FUNDING-CARRY-V1-TRANSFERRED-V2-POLICY',predeclaredDesign:'research/eth-funding-carry-v1-design.md',
    ...
    productionConfig:C,summary,yearStats,gates,validationPass:Object.values(gates).every(Boolean),
  ...
  await fs.writeFile('artifacts/eth-funding-carry-v1-evidence.json',JSON.stringify(out,null,2));
  console.log(JSON.stringify({summary:out.summary,yearStats:out.yearStats,gates:out.gates,validationPass:out.validationPass,cycles:out.cycles,data:out.data},null,2));
