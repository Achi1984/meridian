...
/* v5.45.0 — Entry Intelligence 2.4
...
function gridView(){
 ...
 const activeRisk=card(`<div class="eyebrow">GRID COMMANDER 3.9 ${snapshotBadge('UNIFIED RISK SSOT')}</div><div class="forecast-main">${critical?'ACTIVE RISK':'RISK CONTROL'}</div><div class="grid2">${metric('FUTURES RISK',futuresRisk,critical?'red':danger?'amber':'green')}${metric('CRITICAL',critical,critical?'red':'green')}${metric('RECOVERY',danger,danger?'amber':'green')}${metric('RISK GATE',gate.riskOpen?'OPEN':'BLOCKED',gate.riskOpen?'green':'red')}</div><p class="footer-note">Priorität: Liquidationsrisiko vor neuem Entry.</p>`);
 const bestSetup=p?card(`<div class="eyebrow">BEST SETUP ${liveBadge('BRIDGE')}</div><div class="forecast-main">${best}</div><div class="grid2">${metric('ENTRY',executionPrice(p.low,p.sym)+' – '+executionPrice(p.high,p.sym),'cyan')}${metric('R:R',fmt(p.rr,2)+' : 1',p.rr>=2?'green':'amber')}</div><p class="footer-note">${bridge.reason} · ${p.source||'MODEL'}</p>`):card(`<div class="eyebrow">BEST SETUP ${snapshotBadge('PENDING')}</div><div class="forecast-main amber">ENTRY PENDING</div><p class="footer-note">Keine gültige Preiszone. MERIDIAN zeigt bewusst keinen $0-Entry.</p>`);
 const entryPlan=p?card(`<div class="section-title">ENTRY PLAN · ${p.sym}</div><div class="grid2">${metric('SL',executionPrice(p.stop,p.sym),'red')}${metric('TP1',executionPrice(p.tp1,p.sym),'green')}${metric('TP2',executionPrice(p.tp2,p.sym),'green')}${metric('TP3',executionPrice(p.tp3,p.sym),'green')}${metric('MAX RISK',gate.open?'$'+fmt(bridge.maxRisk,0):'$0 · LOCKED',gate.open?'green':'red')}${metric('STATUS',bridge.status,bridge.tone)}</div><p class="footer-note">Keine automatische Order-Ausführung.</p>`):card(`<div class="section-title">ENTRY PLAN</div><p class="footer-note">ENTRY PENDING · keine gültige GRID/FIB-Preiszone.</p>`);
 ...
 return activeRisk+
   ...
   focus('grid-lifecycle','BOT LIFECYCLE','KEEP · ADD · REDUCE · CLOSE · REBUILD',moduleSafe('BOT LIFECYCLE',()=>lifecycleActionPanel()),false)+
   focus('grid-recovery-best','BEST RECOVERY ACTION','Optimizer · Ziel 8% / 12%',moduleSafe('RECOVERY OPTIMIZER',()=>recoveryOptimizerPanel()),false)+
