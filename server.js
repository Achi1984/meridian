...
function submitSignal(signal){return serializePaper(()=>submitSignalUnlocked(signal));}
...
async function botObserverStatus(){
  const [baselineState,challengerV2,challengerV3,fundingCarry]=await Promise.all([
    loadPaperState(),challengerV2Status(),challengerV3Status(),fundingCarryStatus()
  ]);
  return buildBotObserver({engine:engineStatus(),safety:{paperTrading:config.paperTrading,liveTrading:config.liveTrading},baselineState,challengerV2,challengerV3,fundingCarry,botLifecycle:BOT_LIFECYCLE});
}
