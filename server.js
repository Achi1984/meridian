...
const FUNDING_CARRY_STATE_KEY="funding_carry_v1";
const FUNDING_CARRY_CHECK_MS=5*60000;
async function loadFundingCarryState(){const s=await getState(FUNDING_CARRY_STATE_KEY,null);if(s)return s;const fresh=newFundingCarryPaperState();await setState(FUNDING_CARRY_STATE_KEY,fresh);return fresh;}
async function fundingCarryMarket(){
  const [spot,premium,spotBook,perpBook]=await Promise.all([getTicker(FUNDING_CARRY_PAPER_V1_CONFIG.symbol),fetchJson(`${BINANCE_FUTURES}/fapi/v1/premiumIndex?symbol=${FUNDING_CARRY_PAPER_V1_CONFIG.symbol}`),fetchJson(`${BINANCE}/api/v3/ticker/bookTicker?symbol=${FUNDING_CARRY_PAPER_V1_CONFIG.symbol}`),fetchJson(`${BINANCE_FUTURES}/fapi/v1/ticker/bookTicker?symbol=${FUNDING_CARRY_PAPER_V1_CONFIG.symbol}`)]);
  const perpMarkPrice=Number(premium?.markPrice);if(!(perpMarkPrice>0))throw new Error("invalid BTC perpetual mark");
  return{spotPrice:spot.price,perpMarkPrice,spotBid:Number(spotBook?.bidPrice),spotAsk:Number(spotBook?.askPrice),perpBid:Number(perpBook?.bidPrice),perpAsk:Number(perpBook?.askPrice),nextFundingTime:Number(premium?.nextFundingTime)||null,lastFundingRate:Number(premium?.lastFundingRate)||null};
}
async function recentBtcFunding(now=Date.now(),days=30){
  const startTime=now-days*86400000,j=await fetchJson(`${BINANCE_FUTURES}/fapi/v1/fundingRate?symbol=BTCUSDT&startTime=${startTime}&endTime=${now}&limit=1000`);
  return(Array.isArray(j)?j:[]).map(x=>({fundingTime:Number(x.fundingTime),fundingRate:Number(x.fundingRate),markPrice:Number(x.markPrice)})).filter(x=>x.fundingTime>0&&Number.isFinite(x.fundingRate)&&x.markPrice>0);
}
async function fundingCarryCycle(force=false){
  const now=Date.now();let s=await loadFundingCarryState();
...
async function fundingCarryStatus(){return fundingCarryPaperStatus(await loadFundingCarryState());}

const FUNDING_CARRY_V2_STATE_KEY="funding_carry_v2";
...
async function fundingCarryV2PaperStatus(){return fundingCarryV2Status(await loadFundingCarryV2State());}

async function botObserverStatus(){
...
async function shadowV1Status(){
  ...
  return {enabled:BOT_LIFECYCLE.SHADOW_V1.active,lifecycle:BOT_LIFECYCLE.SHADOW_V1,researchOnly:true,independentLedger:true,baselineRiskGateIgnored:true,ruleset:SHADOW_V1_RULESET,filters:{minTechnical:SHADOW_V1_MIN_TECH,minCandidate:SHADOW_V1_MIN_CAND,longRegime:"BULL",shortRegimes:["BEAR","RANGE"],blockRegime:"TRANSITION",maxEntryDistanceAtr:config.maxEntryDistanceAtr},account:{startEquity:round(a.startEquity,2),cash:round(a.cash,2),equity:round(eq,2),peakEquity:round(peak,2),realizedPnl:round(a.realizedPnl,2),unrealizedPnl:round(a.unrealizedPnl,2),drawdownPct:round(dd,2)},openPositions:op,openCount:op.length,closedCount:tr.length,winRate:tr.length?round(wins.length/tr.length*100,1):0,profitFactor:gl>0?round(gp/gl,2):(gp>0?99:0),recentClosed:tr.slice(-12).reverse(),lastSignal:s.lastSignal||null,lastDecision:s.lastDecision||null,lastEvaluations:s.lastEvaluations||[],lastScanAt:s.lastScanAt||null,updatedAt:s.updatedAt};
}


// MERIDIAN CHALLENGER V2 — research-only adaptive parallel ledger.
// Baseline 6.2 and Shadow V1 remain unchanged. No live execution path.
const CHALLENGER_V2_KEY="challenger_v2";
const CHALLENGER_V2_RULESET="6.99-CHALLENGER-V2";
const CHALLENGER_V2_START=10000;
const CHALLENGER_V2_FULL_RISK_PCT=config.riskPerTradePct;
const CHALLENGER_V2_CAUTION_RISK_PCT=Math.max(.1,round(config.riskPerTradePct*.5,3));
const CHALLENGER_V2_TRADE_SCORE=72;
const CHALLENGER_V2_CAUTION_SCORE=62;
async function loadChallengerV2(){const z=await getState(CHALLENGER_V2_KEY,null);if(z)return rollover(z);const x={account:{startEquity:CHALLENGER_V2_START,cash:CHALLENGER_V2_START,equity:CHALLENGER_V2_START,peakEquity:CHALLENGER_V2_START,dayStartEquity:CHALLENGER_V2_START,dayKey:today(),realizedPnl:0,unrealizedPnl:0},positions:[],trades:[],equityCurve:[{ts:new Date().toISOString(),equity:CHALLENGER_V2_START}],counterfactuals:[],lastSignal:null,lastDecision:null,lastEvaluations:[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),ruleset:CHALLENGER_V2_RULESET};await saveChallengerV2(x);return x;}
async function saveChallengerV2(x){x.updatedAt=new Date().toISOString();await setState(CHALLENGER_V2_KEY,x);}
function challengerRegimeAdjustment(side,regime){if(side==="LONG"){if(regime==="BULL")return 10;if(regime==="BEAR")return-12;if(regime==="RANGE")return-2;return-5;}if(regime==="BEAR")return 10;if(regime==="RANGE")return 4;if(regime==="BULL")return-12;return-5;}
function challengerDecision(signal){const regime=shadowRegime(signal),technical=Number(signal.technical||0),candidate=Number(signal.candidate||0),distanceAtr=Number(signal.distanceAtr),distanceQuality=Number.isFinite(distanceAtr)?clamp(100-(distanceAtr/Math.max(config.maxEntryDistanceAtr,.01))*35,0,100):40,regimeAdj=challengerRegimeAdjustment(String(signal.side||"").toUpperCase(),regime),confidence=clamp(Math.round(technical*.42+candidate*.38+distanceQuality*.20+regimeAdj),0,100);let decision="SKIP",riskPct=0,reasons=[];if(signal.status!=="READY")reasons.push(signal.status==="WAIT_ENTRY_ZONE"?"WAIT_ENTRY_ZONE":"BASE_NOT_READY");else if(confidence>=CHALLENGER_V2_TRADE_SCORE){decision="TRADE";riskPct=CHALLENGER_V2_FULL_RISK_PCT;}else if(confidence>=CHALLENGER_V2_CAUTION_SCORE){decision="CAUTION";riskPct=CHALLENGER_V2_CAUTION_RISK_PCT;}else reasons.push("CONFIDENCE_LT_CAUTION");return{symbol:String(signal.symbol||"").toUpperCase(),side:String(signal.side||"").toUpperCase(),status:signal.status||null,decision,riskPct,confidence,technical,candidate,distanceAtr:Number.isFinite(distanceAtr)?distanceAtr:null,distanceQuality:round(distanceQuality,1),regime,regimeAdjustment:regimeAdj,reasons,evaluatedAt:new Date().toISOString(),researchOnly:true};}
async function observeChallengerV2Scan(candidates){const s=rollover(await loadChallengerV2()),obs=(candidates||[]).map(challengerDecision);s.lastEvaluations=obs.slice(0,10);s.lastDecision=obs[0]||null;s.lastScanAt=new Date().toISOString();for(let i=0;i<(candidates||[]).length;i++){const c=candidates[i],d=obs[i];if(!d||d.decision!=="SKIP"||!(Number(c.entry)>0&&Number(c.sl)>0))continue;if((s.counterfactuals||[]).some(x=>x.status==="OPEN"&&x.symbol===d.symbol&&x.side===d.side))continue;s.counterfactuals=s.counterfactuals||[];s.counterfactuals.push({id:crypto.randomUUID(),symbol:d.symbol,side:d.side,status:"OPEN",decision:"SKIP",confidence:d.confidence,regime:d.regime,technical:d.technical,candidate:d.candidate,distanceAtr:d.distanceAtr,entry:+c.entry,sl:+c.sl,tp1:c.tp1==null?null:+c.tp1,tp2:c.tp2==null?null:+c.tp2,openedAt:new Date().toISOString(),reason:d.reasons[0]||"SKIP",ruleset:CHALLENGER_V2_RULESET});}if(s.counterfactuals.length>1000)s.counterfactuals=s.counterfactuals.slice(-1000);await saveChallengerV2(s);return obs;}
function challengerRiskGate({account,positions,tradesToday,signal,riskPct}){const r=[];if(!signal||!["LONG","SHORT"].includes(signal.side))r.push("NO_DIRECTION");if(!(Number(signal.entry)>0&&Number(signal.sl)>0))r.push("INVALID_ENTRY_OR_SL");if(positions.length>=config.maxOpenPositions)r.push("MAX_OPEN_POSITIONS");if(tradesToday>=config.maxTradesPerDay)r.push("MAX_TRADES_PER_DAY");const o=openRisk(positions);if(o+riskPct>config.maxPortfolioRiskPct+1e-9)r.push("MAX_PORTFOLIO_RISK");const eq=Number(account.equity||CHALLENGER_V2_START),day=Number(account.dayStartEquity||eq),peak=Number(account.peakEquity||eq),dl=day>0?Math.max(0,(day-eq)/day*100):0,dd=peak>0?Math.max(0,(peak-eq)/peak*100):0;if(dl>=config.maxDailyLossPct)r.push("MAX_DAILY_LOSS");if(dd>=config.maxDrawdownPct)r.push("MAX_DRAWDOWN");return{ok:r.length===0,reasons:r,openRiskPct:o,dailyLossPct:dl,drawdownPct:dd};}
function openChallengerPosition({signal,account,riskPct,decision}){const entry=slip(+signal.entry,signal.side,true),d=Math.abs(entry-(+signal.sl)),qty=d>0?account.equity*(riskPct/100)/d:0;if(!(qty>0))throw new Error("challenger position size <=0");const fee=qty*entry*config.feeBps/10000;return{id:crypto.randomUUID(),symbol:signal.symbol,side:signal.side,status:"OPEN",entry,qty,sl:+signal.sl,tp1:+signal.tp1||null,tp2:+signal.tp2||null,openedAt:new Date().toISOString(),riskPct,feeOpen:fee,source:"MERIDIAN-CHALLENGER-V2",ruleset:CHALLENGER_V2_RULESET,technical:signal.technical??null,candidate:signal.candidate??null,distanceAtr:signal.distanceAtr??null,challengerDecision:decision.decision,challengerConfidence:decision.confidence,challengerRegime:decision.regime};}
async function submitChallengerV2(signal){const s=rollover(await loadChallengerV2()),decision=challengerDecision(signal),n={...signal,symbol:String(signal.symbol||"").toUpperCase(),side:String(signal.side||"").toUpperCase(),entry:+signal.entry,sl:+signal.sl,tp1:signal.tp1==null?null:+signal.tp1,tp2:signal.tp2==null?null:+signal.tp2,source:"MERIDIAN-CHALLENGER-V2",ruleset:CHALLENGER_V2_RULESET};s.lastSignal={...n,receivedAt:new Date().toISOString(),challenger:decision};if(!["TRADE","CAUTION"].includes(decision.decision)){await saveChallengerV2(s);return{accepted:false,reasons:decision.reasons,decision};}if(s.positions.some(p=>p.status==="OPEN"&&p.symbol===n.symbol))return{accepted:false,reasons:["SAME_SYMBOL_POSITION_OPEN"],decision};const cutoff=Date.now()-config.cooldownMinutes*60000;if(s.trades.some(t=>t.symbol===n.symbol&&new Date(t.closedAt||t.openedAt||0).getTime()>=cutoff))return{accepted:false,reasons:["SYMBOL_COOLDOWN"],decision};const gate=challengerRiskGate({account:s.account,positions:s.positions.filter(p=>p.status==="OPEN"),tradesToday:dayTradeCount(s),signal:n,riskPct:decision.riskPct});if(!gate.ok){s.lastSignal.gate=gate;await saveChallengerV2(s);return{accepted:false,reasons:gate.reasons,gate,decision};}const p=openChallengerPosition({signal:n,account:s.account,riskPct:decision.riskPct,decision});s.positions.push(p);s.account.cash-=p.feeOpen;s.account.realizedPnl-=p.feeOpen;s.lastSignal.decision=decision.decision;s.lastSignal.reasons=[];await saveChallengerV2(s);await addEvent("CHALLENGER_V2_POSITION_OPENED",{id:p.id,symbol:p.symbol,side:p.side,decision:p.challengerDecision,confidence:p.challengerConfidence,regime:p.challengerRegime,riskPct:p.riskPct,entry:p.entry,sl:p.sl,tp1:p.tp1});return{accepted:true,position:p,gate,decision};}
async function challengerV2Cycle(m){let s=rollover(await loadChallengerV2());const next=[],closed=[];let unreal=0;for(const p of s.positions){if(p.status!=="OPEN")continue;const q=m.quotes[p.symbol];if(!q||Date.now()-q.ts>config.marketStaleMs){next.push(p);continue;}const marked=markPosition(p,q.price),reason=exitReason(marked,q.price);if(reason){const done=closePaperPosition(marked,q.price,reason);done.ruleset=CHALLENGER_V2_RULESET;s.account.cash+=done.realized+p.feeOpen;s.account.realizedPnl+=done.realized+p.feeOpen;s.trades.push(done);closed.push(done);}else{unreal+=marked.unrealized;next.push(marked);}}s.positions=next;s.account.unrealizedPnl=unreal;s.account.equity=s.account.cash+unreal;s.account.peakEquity=Math.max(s.account.peakEquity||s.account.equity,s.account.equity);const le=last(s.equityCurve);if(!le||Date.now()-new Date(le.ts).getTime()>=60000){s.equityCurve.push({ts:new Date().toISOString(),equity:s.account.equity});if(s.equityCurve.length>50000)s.equityCurve=s.equityCurve.slice(-50000);}for(const cf of(s.counterfactuals||[])){if(cf.status!=="OPEN")continue;const q=m.quotes[cf.symbol];if(!q||Date.now()-q.ts>config.marketStaleMs)continue;const reason=exitReason(cf,q.price);if(!reason)continue;cf.status="CLOSED";cf.exit=q.price;cf.closedAt=new Date().toISOString();cf.exitReason=reason;const dir=cf.side==="LONG"?1:-1,r=Math.abs(cf.entry-cf.sl)||1;cf.outcomeR=round((q.price-cf.entry)*dir/r,3);}await saveChallengerV2(s);for(const c of closed)await addEvent("CHALLENGER_V2_POSITION_CLOSED",{symbol:c.symbol,side:c.side,decision:c.challengerDecision,confidence:c.challengerConfidence,regime:c.challengerRegime,riskPct:c.riskPct,realized:round(c.realized,2),exitReason:c.exitReason});}
async function challengerV2Status(){const s=await loadChallengerV2(),a=s.account||{},tr=s.trades||[],op=(s.positions||[]).filter(x=>x.status==="OPEN"),cf=s.counterfactuals||[],wins=tr.filter(x=>Number(x.realized)>0),losses=tr.filter(x=>Number(x.realized)<0),gp=wins.reduce((q,x)=>q+Number(x.realized||0),0),gl=Math.abs(losses.reduce((q,x)=>q+Number(x.realized||0),0)),peak=Number(a.peakEquity||a.equity||CHALLENGER_V2_START),eq=Number(a.equity||CHALLENGER_V2_START),dd=peak>0?Math.max(0,(peak-eq)/peak*100):0,cfClosed=cf.filter(x=>x.status==="CLOSED");return{enabled:true,researchOnly:true,independentLedger:true,executionImpact:false,ruleset:CHALLENGER_V2_RULESET,model:{softScoring:true,decisions:["TRADE","CAUTION","SKIP"],tradeScore:CHALLENGER_V2_TRADE_SCORE,cautionScore:CHALLENGER_V2_CAUTION_SCORE,fullRiskPct:CHALLENGER_V2_FULL_RISK_PCT,cautionRiskPct:CHALLENGER_V2_CAUTION_RISK_PCT,weights:{technical:.42,candidate:.38,entryDistance:.20},regimeAdaptive:true},account:{startEquity:round(a.startEquity,2),cash:round(a.cash,2),equity:round(eq,2),peakEquity:round(peak,2),realizedPnl:round(a.realizedPnl,2),unrealizedPnl:round(a.unrealizedPnl,2),drawdownPct:round(dd,2)},openPositions:op,openCount:op.length,closedCount:tr.length,winRate:tr.length?round(wins.length/tr.length*100,1):0,profitFactor:gl>0?round(gp/gl,2):(gp>0?99:0),recentClosed:tr.slice(-12).reverse(),lastSignal:s.lastSignal||null,lastDecision:s.lastDecision||null,lastEvaluations:s.lastEvaluations||[],counterfactual:{tracked:cf.length,open:cf.filter(x=>x.status==="OPEN").length,closed:cfClosed.length,positiveR:cfClosed.filter(x=>Number(x.outcomeR)>0).length,negativeR:cfClosed.filter(x=>Number(x.outcomeR)<0).length,recent:cfClosed.slice(-12).reverse()},lastScanAt:s.lastScanAt||null,updatedAt:s.updatedAt};}

// Challenger V3 is created once from the sealed V2 stop analysis. Its parameter
// snapshot never changes afterwards; its ledger is independent and prospective.
const CHALLENGER_V3_KEY="challenger_v3";
const CHALLENGER_V3_RULESET="8.27-CHALLENGER-V3-POST-STOP";
const challengerV2Params=()=>({tradeScore:CHALLENGER_V2_TRADE_SCORE,cautionScore:CHALLENGER_V2_CAUTION_SCORE,fullRiskPct:CHALLENGER_V2_FULL_RISK_PCT});
// Serialize mutations in the single core process; readers receive detached snapshots.
let challengerV3Queue=Promise.resolve();
function serializeChallengerV3(action){const next=challengerV3Queue.then(action);challengerV3Queue=next.catch(()=>{});return next;}
function ensureChallengerV3(){return serializeChallengerV3(ensureChallengerV3Unlocked);}
function observeChallengerV3Scan(candidates){return serializeChallengerV3(()=>observeChallengerV3ScanUnlocked(candidates));}
function submitChallengerV3(signal){return serializeChallengerV3(()=>submitChallengerV3Unlocked(signal));}
function challengerV3Cycle(m){return serializeChallengerV3(()=>challengerV3CycleUnlocked(m));}
async function ensureChallengerV3Unlocked(){
  const existing=await getState(CHALLENGER_V3_KEY,null);if(existing)return rollover(existing);
  const parent=await loadChallengerV2();
  const plan=buildSuccessorPlan(parent,config,challengerV2Params());
  if(!plan.eligible)return null;
  const now=new Date().toISOString(),start=CHALLENGER_V2_START;
  const state={
    account:{startEquity:start,cash:start,equity:start,peakEquity:start,dayStartEquity:start,dayKey:today(),realizedPnl:0,unrealizedPnl:0},
    positions:[],trades:[],equityCurve:[{ts:now,equity:start}],counterfactuals:[],lastSignal:null,lastDecision:null,lastEvaluations:[],
    lifecycle:{status:"ACTIVE_PAPER",generation:3,parent:"CHALLENGER_V2",createdAt:now,trigger:"MAX_DRAWDOWN",autoPromotion:false},
    analysis:plan.analysis,parameters:plan.parameters,createdAt:now,updatedAt:now,ruleset:CHALLENGER_V3_RULESET
  };
  await setState(CHALLENGER_V3_KEY,state);
  await addEvent("CHALLENGER_V3_STARTED",{ruleset:CHALLENGER_V3_RULESET,parent:"CHALLENGER_V2",trigger:"MAX_DRAWDOWN",parameters:plan.parameters,analysis:{closedTrades:plan.analysis.performance.closedTrades,pnl:plan.analysis.performance.pnl,profitFactor:plan.analysis.performance.profitFactor,drawdownPct:plan.analysis.maximumDrawdownPct,causes:plan.analysis.causes}});
  return state;
}
...
function challengerV3Decision(signal,parameters){
  ...
  const regimeAdj=challengerRegimeAdjustment(String(signal.side||"").toUpperCase(),regime);
  const confidence=clamp(Math.round(technical*parameters.weights.technical+candidate*parameters.weights.candidate+distanceQuality*parameters.weights.entryDistance+regimeAdj),0,100);
  let decision="SKIP",riskPct=0,reasons=[];
  if(signal.status!=="READY")reasons.push(signal.status==="WAIT_ENTRY_ZONE"?"WAIT_ENTRY_ZONE":"BASE_NOT_READY");
  else if(confidence>=parameters.tradeScore){decision="TRADE";riskPct=parameters.fullRiskPct;}
  else if(confidence>=parameters.cautionScore){decision="CAUTION";riskPct=parameters.cautionRiskPct;}
...
function challengerV3RiskGate({state,signal,riskPct}){
  const p=state.parameters,a=state.account,positions=state.positions.filter(x=>x.status==="OPEN"),r=[];
  if(!signal||!["LONG","SHORT"].includes(signal.side))r.push("NO_DIRECTION");
  if(!config.symbols.includes(signal.symbol))r.push("SYMBOL_NOT_ALLOWED");
  ...
  const eq=Number(a.equity??CHALLENGER_V2_START),day=Number(a.dayStartEquity??eq),peak=Number(a.peakEquity??eq);
  const dailyLossPct=day>0?Math.max(0,(day-eq)/day*100):0,drawdownPct=peak>0?Math.max(0,(peak-eq)/peak*100):0;
  if(!Number.isFinite(eq)||eq<=0)r.push("INVALID_EQUITY");
  if(dailyLossPct>=p.maxDailyLossPct)r.push("MAX_DAILY_LOSS");if(drawdownPct>=p.maxDrawdownPct)r.push("MAX_DRAWDOWN");
...
async function submitChallengerV3Unlocked(signal){
  ...
  const n={...signal,symbol:String(signal.symbol||"").toUpperCase(),side:String(signal.side||"").toUpperCase(),entry:+signal.entry,sl:+signal.sl,tp1:signal.tp1==null?null:+signal.tp1,tp2:signal.tp2==null?null:+signal.tp2};
  ...
  if(age<s.parameters.cooldownMinutes*60000)return reject("SYMBOL_COOLDOWN");
  ...
  if(typeof evidenceSnapshot==="function")position.evidenceSnapshot=evidenceSnapshot(n,position);
  ...
  await addEvent("CHALLENGER_V3_POSITION_OPENED",{id:position.id,symbol:position.symbol,side:position.side,riskPct:position.riskPct,confidence:position.challengerConfidence,parameterVersion:position.parameterVersion});
  try{if(position.evidenceSnapshot&&typeof saveEvidence==="function"){await saveEvidence(position.evidenceSnapshot);await addEvent("CHALLENGER_V3_EVIDENCE_CAPTURED",{positionId:position.id,symbol:position.symbol,side:position.side,captureVersion:"8.40-V3-EVIDENCE"});}}catch(e){await addEvent("CHALLENGER_V3_EVIDENCE_ERROR",{positionId:position.id,message:String(e?.message||e)}).catch(()=>{});}
  try{await pairDirectionalV4(position);}catch(e){await addEvent("CHALLENGER_V4_ERROR",{stage:"pair",parentPositionId:position.id,message:String(e?.message||e)}).catch(()=>{});}
  return{accepted:true,position,gate,decision};
}
async function challengerV3CycleUnlocked(m){
  ...
  await saveChallengerV3(s);for(const c of closed){await addEvent("CHALLENGER_V3_POSITION_CLOSED",{symbol:c.symbol,side:c.side,realized:round(c.realized,2),exitReason:c.exitReason,parameterVersion:c.parameterVersion});try{await closeEvidence(c.id,{closedAt:c.closedAt,exitReason:c.exitReason,exit:round(c.exit,8),realized:round(c.realized,8),side:c.side,symbol:c.symbol,ruleset:c.ruleset});}catch(e){await addEvent("CHALLENGER_V3_EVIDENCE_ERROR",{positionId:c.id,stage:"close",message:String(e?.message||e)}).catch(()=>{});}}
  try{await directionalV4Cycle(m);}catch(e){await addEvent("CHALLENGER_V4_ERROR",{stage:"cycle",message:String(e?.message||e)}).catch(()=>{});}
}
...
const DIRECTIONAL_V4_KEY="challenger_v4_exit_shadow";
let directionalV4Queue=Promise.resolve();
function serializeDirectionalV4(action){const next=directionalV4Queue.then(action);directionalV4Queue=next.catch(()=>{});return next;}
async function loadDirectionalV4State(){const s=await getState(DIRECTIONAL_V4_KEY,null);if(s)return structuredClone(s);const fresh=newDirectionalV4State(Date.now(),{feeBps:config.feeBps,slippageBps:config.slippageBps});await setState(DIRECTIONAL_V4_KEY,fresh);return fresh;}
async function pairDirectionalV4(parent){return serializeDirectionalV4(async()=>{let s=await loadDirectionalV4State();if(s.lifecycle?.status!=="PAIRED_SHADOW")return s;if(s.frozenPolicy.feeBps!==config.feeBps||s.frozenPolicy.slippageBps!==config.slippageBps)throw new Error("V4 cost config drift");s=pairDirectionalV4Position(s,parent);await setState(DIRECTIONAL_V4_KEY,s);await addEvent("CHALLENGER_V4_PAIR_OPENED",{parentPositionId:parent.id,symbol:parent.symbol,side:parent.side,ruleset:s.ruleset});return s;});}
async function directionalV4Cycle(m){return serializeDirectionalV4(async()=>{let s=await loadDirectionalV4State();const before=new Set((s.trades||[]).map(x=>x.id));s=cycleDirectionalV4(s,m?.quotes||{});const v3=await loadChallengerV3();for(const parent of(v3?.trades||[]))s=recordDirectionalV3Close(s,parent);await setState(DIRECTIONAL_V4_KEY,s);for(const x of(s.trades||[]))if(!before.has(x.id))await addEvent("CHALLENGER_V4_PAIR_CLOSED",{parentPositionId:x.parentPositionId,symbol:x.symbol,side:x.side,realized:round(x.realized,2),exitReason:x.exitReason,ruleset:s.ruleset});return s;});}
async function directionalV4PaperStatus(){return serializeDirectionalV4(async()=>directionalV4Status(await loadDirectionalV4State()));}

// MERIDIAN REGIME V1 — research-only adaptive strategy selector.
const REGIME_V1_KEY="regime_v1";
const REGIME_V1_START=10000;
async function loadRegimeV1(){const z=await getState(REGIME_V1_KEY,null);if(z)return rollover(z);const x={account:{startEquity:REGIME_V1_START,cash:REGIME_V1_START,equity:REGIME_V1_START,peakEquity:REGIME_V1_START,dayStartEquity:REGIME_V1_START,dayKey:today(),realizedPnl:0,unrealizedPnl:0},positions:[],trades:[],equityCurve:[{ts:new Date().toISOString(),equity:REGIME_V1_START}],lastSignal:null,lastDecision:null,lastEvaluations:[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),ruleset:REGIME_V1_RULESET};await saveRegimeV1(x);return x;}
async function saveRegimeV1(x){x.updatedAt=new Date().toISOString();await setState(REGIME_V1_KEY,x);}
function regimeModel(signal){return regimeDecision(signal,{fullRiskPct:config.riskPerTradePct,cautionRiskPct:Math.max(.1,round(config.riskPerTradePct*.5,3))});}
async function observeRegimeV1Scan(candidates){const s=rollover(await loadRegimeV1()),obs=(candidates||[]).map(regimeModel).sort((a,b)=>b.score-a.score);s.lastEvaluations=obs.slice(0,10);s.lastDecision=obs[0]||null;s.lastScanAt=new Date().toISOString();await saveRegimeV1(s);return obs;}
function openRegimePosition(signal,account,d){const entry=slip(+d.entry,d.side,true),dist=Math.abs(entry-(+d.sl)),qty=dist>0?account.equity*(d.riskPct/100)/dist:0;if(!(qty>0))throw new Error("regime position size <=0");const fee=qty*entry*config.feeBps/10000;return{id:crypto.randomUUID(),symbol:String(signal.symbol||"").toUpperCase(),side:d.side,status:"OPEN",entry,qty,sl:+d.sl,tp1:+d.tp1||null,tp2:+d.tp2||null,openedAt:new Date().toISOString(),riskPct:d.riskPct,feeOpen:fee,source:"MERIDIAN-REGIME-V1",ruleset:REGIME_V1_RULESET,technical:signal.technical??null,candidate:signal.candidate??null,distanceAtr:d.distanceAtr??null,regimeType:d.regime,regimeScore:d.score,regimeDecision:d.decision,sourceSide:d.sourceSide,entryQuality:d.entryQuality,momentumFit:d.momentumFit};}
async function submitRegimeV1(signal){const s=rollover(await loadRegimeV1()),d=regimeModel(signal);s.lastSignal={...d,receivedAt:new Date().toISOString()};if(!["TRADE","CAUTION"].includes(d.decision)){await saveRegimeV1(s);return{accepted:false,reasons:d.reasons,decision:d};}if(s.positions.some(p=>p.status==="OPEN"&&p.symbol===d.symbol))return{accepted:false,reasons:["SAME_SYMBOL_POSITION_OPEN"],decision:d};const cutoff=Date.now()-config.cooldownMinutes*60000;if(s.trades.some(t=>t.symbol===d.symbol&&new Date(t.closedAt||t.openedAt||0).getTime()>=cutoff))return{accepted:false,reasons:["SYMBOL_COOLDOWN"],decision:d};const tradeSignal={...signal,symbol:d.symbol,side:d.side,entry:d.entry,sl:d.sl,tp1:d.tp1,tp2:d.tp2};const gate=challengerRiskGate({account:s.account,positions:s.positions.filter(p=>p.status==="OPEN"),tradesToday:dayTradeCount(s),signal:tradeSignal,riskPct:d.riskPct});if(!gate.ok){s.lastSignal.gate=gate;await saveRegimeV1(s);return{accepted:false,reasons:gate.reasons,gate,decision:d};}const p=openRegimePosition(signal,s.account,d);s.positions.push(p);s.account.cash-=p.feeOpen;s.account.realizedPnl-=p.feeOpen;await saveRegimeV1(s);await addEvent("REGIME_V1_POSITION_OPENED",{id:p.id,symbol:p.symbol,side:p.side,regime:p.regimeType,score:p.regimeScore,riskPct:p.riskPct,entry:p.entry,sl:p.sl,tp1:p.tp1});return{accepted:true,position:p,gate,decision:d};}
async function regimeV1Cycle(m){let s=rollover(await loadRegimeV1());const next=[],closed=[];let unreal=0;for(const p of s.positions){if(p.status!=="OPEN")continue;const q=m.quotes[p.symbol];if(!q||Date.now()-q.ts>config.marketStaleMs){next.push(p);continue;}const marked=markPosition(p,q.price),reason=exitReason(marked,q.price);if(reason){const done=closePaperPosition(marked,q.price,reason);done.ruleset=REGIME_V1_RULESET;done.regimeType=p.regimeType;done.regimeScore=p.regimeScore;s.account.cash+=done.realized+p.feeOpen;s.account.realizedPnl+=done.realized+p.feeOpen;s.trades.push(done);closed.push(done);}else{unreal+=marked.unrealized;next.push(marked);}}s.positions=next;s.account.unrealizedPnl=unreal;s.account.equity=s.account.cash+unreal;s.account.peakEquity=Math.max(s.account.peakEquity||s.account.equity,s.account.equity);const le=last(s.equityCurve);if(!le||Date.now()-new Date(le.ts).getTime()>=60000){s.equityCurve.push({ts:new Date().toISOString(),equity:s.account.equity});if(s.equityCurve.length>50000)s.equityCurve=s.equityCurve.slice(-50000);}await saveRegimeV1(s);for(const c of closed)await addEvent("REGIME_V1_POSITION_CLOSED",{symbol:c.symbol,side:c.side,regime:c.regimeType,score:c.regimeScore,riskPct:c.riskPct,realized:round(c.realized,2),exitReason:c.exitReason});}
async function regimeV1Status(){const s=await loadRegimeV1(),a=s.account||{},tr=s.trades||[],op=(s.positions||[]).filter(x=>x.status==="OPEN"),wins=tr.filter(x=>Number(x.realized)>0),losses=tr.filter(x=>Number(x.realized)<0),gp=wins.reduce((q,x)=>q+Number(x.realized||0),0),gl=Math.abs(losses.reduce((q,x)=>q+Number(x.realized||0),0)),peak=Number(a.peakEquity||a.equity||REGIME_V1_START),eq=Number(a.equity||REGIME_V1_START),dd=peak>0?Math.max(0,(peak-eq)/peak*100):0;return{enabled:BOT_LIFECYCLE.REGIME_V1.active,lifecycle:BOT_LIFECYCLE.REGIME_V1,researchOnly:true,independentLedger:true,executionImpact:false,ruleset:REGIME_V1_RULESET,model:{strategySelector:true,modes:["TREND_UP","TREND_DOWN","RANGE","EXPANSION","CHOP"],tradeScore:REGIME_V1_CONFIG.tradeScore,cautionScore:REGIME_V1_CONFIG.cautionScore,weights:REGIME_V1_CONFIG.weights,riskMultipliers:REGIME_V1_CONFIG.riskMultipliers},account:{startEquity:round(a.startEquity,2),cash:round(a.cash,2),equity:round(eq,2),peakEquity:round(peak,2),realizedPnl:round(a.realizedPnl,2),unrealizedPnl:round(a.unrealizedPnl,2),drawdownPct:round(dd,2)},openPositions:op,openCount:op.length,closedCount:tr.length,winRate:tr.length?round(wins.length/tr.length*100,1):0,profitFactor:gl>0?round(gp/gl,2):(gp>0?99:0),recentClosed:tr.slice(-12).reverse(),lastSignal:s.lastSignal||null,lastDecision:s.lastDecision||null,lastEvaluations:s.lastEvaluations||[],lastScanAt:s.lastScanAt||null,updatedAt:s.updatedAt};}
