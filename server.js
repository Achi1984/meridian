...
async function fundingCarryMarket(){
  const [spot,premium]=await Promise.all([getTicker(FUNDING_CARRY_PAPER_V1_CONFIG.symbol),fetchJson(`${BINANCE_FUTURES}/fapi/v1/premiumIndex?symbol=${FUNDING_CARRY_PAPER_V1_CONFIG.symbol}`)]);
  const perpMarkPrice=Number(premium?.markPrice);if(!(perpMarkPrice>0))throw new Error("invalid BTC perpetual mark");
  return{spotPrice:spot.price,perpMarkPrice,nextFundingTime:Number(premium?.nextFundingTime)||null,lastFundingRate:Number(premium?.lastFundingRate)||null};
}
...
async function fundingCarryCycle(force=false){
  ...
  await setState(FUNDING_CARRY_STATE_KEY,s);return s;
}
...
async function challengerV3CycleUnlocked(m){
  ...
  await saveChallengerV3(s);for(const c of closed)await addEvent("CHALLENGER_V3_POSITION_CLOSED",{symbol:c.symbol,side:c.side,realized:round(c.realized,2),exitReason:c.exitReason,parameterVersion:c.parameterVersion});
}
