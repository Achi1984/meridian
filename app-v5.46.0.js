...
/* v5.46.0 — Entry Intelligence 2.4
...
function gridView(){
 ...
 const recovery=topBot&&Number.isFinite(Number(topBot.buffer))&&Number(topBot.buffer)<12
   ? card(`<div class="section-head compact-head"><div><div class="section-title">RECOVERY</div><div class="section-note">${topBot.id} · ${fmt(topBot.buffer,2)}%</div></div><span class="tag ${statusTone}">${topBot.guard?.label||statusLabel}</span></div>
      <div class="recovery-compact">
        <div><span>BEST ACTION</span><b>${bestAction}</b></div>
        <div><span>MODELL DANACH</span><b class="green">${best8?fmt(best8.after,2)+'%':'—'}</b></div>
        <div><span>ZIEL</span><b>≥8% MIN</b></div>
      </div>
      <p class="footer-note">Modellwert, keine Ordergröße. Nach manueller Pionex-Änderung Liquidationspreis neu synchronisieren.</p>`)
   : '';
