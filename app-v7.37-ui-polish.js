...
(function(){
  ...
  const LOGO_SRC='assets/achi-meridian-topbar.webp?v=7.60-R12';
  ...
  function installStyle(){
    ...
    s.textContent=`
      ...
      .bottom .inner,.bottom .inner.five-tabs{grid-template-columns:repeat(6,minmax(0,1fr))!important;gap:2px!important;align-items:stretch!important}
      .bottom .inner .nav,.bottom .inner.five-tabs .nav{min-width:0!important;width:100%!important;padding:7px 0!important;font-size:7.5px!important;text-align:center!important}
      ...
      @media(max-width:650px){
        ...
        .bottom .inner,.bottom .inner.five-tabs{grid-template-columns:repeat(6,minmax(0,1fr))!important;padding-left:2px!important;padding-right:2px!important}
        .bottom .inner .nav,.bottom .inner.five-tabs .nav{font-size:7px!important;padding:7px 0!important}
        .bottom .nav span{font-size:12px!important}
      }
      @media(max-width:390px){
        #${BRAND_IMG_ID}{max-height:68px!important}
        .bottom .inner .nav,.bottom .inner.five-tabs .nav{font-size:6.6px!important}
  ...
  function installBrand(){
    ...
    if(!img){
      ...
      img.id=BRAND_IMG_ID;img.alt='ACHI MERIDIAN';img.decoding='async';img.src=LOGO_SRC;brandline.appendChild(img);
    }else if(img.getAttribute('src')!==LOGO_SRC){img.src=LOGO_SRC}
  ...
  function start(){apply();const root=document.body;if(root&&typeof MutationObserver!=='undefined')new MutationObserver(apply).observe(root,{childList:true,subtree:true})}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
  addEventListener('pageshow',apply);addEventListener('resize',apply);
})();
